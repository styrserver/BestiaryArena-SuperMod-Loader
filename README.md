...
#### Better Bestiary
Enhances the Bestiary sell modal with creature selection presets and temporary overlay toggles:
- **Creature Selector** presets: Already Shiny, Already Awakened, and Already A/S select unlocked duplicates when you already own that milestone per species
- **Overlays** column: temporary session toggles for Maxed, Sealed, and Shinies borders (Mod Settings styling; restores when the modal closes)
- Activated title and author credit in the creature details column
- EN/PT labels and hover tooltips on all controls
...
Automates rune recycling with batch processing and comprehensive statistics (by [btlucas](https://bestiaryarena.com/profile/btlucas)):
...
- View summary and leaderboard in the modal; load a saved setup to replay challenges
...
These are the core mods that provide essential gameplay enhancements. All 10 Official Mods listed below are included by default:
...
#### Usage List
Tabbed modal showing how often creatures and equipment appear across each map's saved Auto-setup (not Setup Manager's separate named-setup library, unless also saved as that map's Auto-setup; event/raid maps aren't counted):
- Creatures and Equipment tabs in one window
- S/A/B/C usage tiers based on how many maps' Auto-setup include that creature/equipment
- Equipment with more than one stat variant (AD/AP/HP) is tracked as separate entries, one per stat
- Hover a portrait for usage stats and the maps it's used on, grouped and ordered by region (no click needed)
