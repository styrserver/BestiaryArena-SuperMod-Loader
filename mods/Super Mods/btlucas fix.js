...
let openRewardsObserver = null;
let focusEventListeners = null;
let currentCountdownTask = null;
let gameStateUnsubscribers = [];

// Manual mode farming control
let manualModeFarmingStarted = false;
...
const subscribeToGameState = () => {
  try {
    ...
    // Subscribe to gameTimer for victory and defeat detection
    if (globalThis.state && globalThis.state.gameTimer) {
      let previousGameState = 'initial';
      
      const gameTimerUnsubscribe = globalThis.state.gameTimer.subscribe(async (state) => {
        const currentGameState = state.context.state;
        
        // Check if game just ended with victory
        if (config.stopAfterVictory && currentGameState === 'victory' && previousGameState !== 'victory') {
          // Skip victory handling if in sandbox mode
          if (isSandboxMode()) {
            console.log('[Bestiary Automator] Victory detected in sandbox mode - ignoring');
            return;
          }
          
          console.log('[Bestiary Automator] Victory state detected via gameTimer!');
          
          // Use manual mode farming victory handler if manual mode farming is active
          if (config.manualModeFarming && manualModeFarmingStarted) {
            handleVictoryInManualMode();
          } else {
            await stopGameAfterVictory();
          }
        }
        
        // Check if game just ended with defeat
        if (currentGameState === 'defeat' && previousGameState !== 'defeat') {
          // Skip defeat handling if in sandbox mode
          if (isSandboxMode()) {
            console.log('[Bestiary Automator] Defeat detected in sandbox mode - ignoring');
            return;
          }
          
          console.log('[Bestiary Automator] Defeat state detected via gameTimer!');
          
          // Close loot summary window after defeat
          setTimeout(() => {
            closeLootSummaryWindow();
          }, 1000);
        }
      ...
      gameStateUnsubscribers.push(gameTimerUnsubscribe);
    }
    
    // Set up MutationObserver for real-time toast detection and skip button detection
    if (!config.autoPlayAfterDefeat && !config.manualModeFarming) return;
        ...
        if (mutation.type === 'childList') {
          ...
          for (const node of mutation.addedNodes) {
            if (node.nodeType === Node.ELEMENT_NODE) {
              ...
              // Check for skip button (manual mode farming)
              if (config.manualModeFarming && isSkipButton(node)) {
                console.log('[Bestiary Automator] Skip button detected via MutationObserver!');
                // Process immediately without debouncing
                handleSkipButton();
                return; // Exit early since we found a skip button
              }
              
              // Check for defeat toast (manual mode farming)
              if (config.manualModeFarming && isDefeatToast(node)) {
                console.log('[Bestiary Automator] Defeat toast detected via MutationObserver for manual mode farming!');
                // Process immediately without debouncing
                handleDefeatInManualMode();
                return; // Exit early since we found a defeat toast
              }
              
              // Also check for skip button in child elements (manual mode farming)
              if (config.manualModeFarming && node.querySelector) {
                const skipButtonsInChildren = node.querySelectorAll('button');
                for (const button of skipButtonsInChildren) {
                  if (isSkipButton(button)) {
                    console.log('[Bestiary Automator] Skip button detected in child elements via MutationObserver!');
                    handleSkipButton();
                    return; // Exit early since we found a skip button
                  }
                }
              ...
              // Also check for defeat toast in child elements (manual mode farming)
              if (config.manualModeFarming && node.querySelector) {
                const defeatToastInChildren = node.querySelector('div.widget-bottom.pixel-font-16.flex.items-center.gap-2.px-2.py-1.text-whiteHighlight:has(img[alt="no"])');
                if (defeatToastInChildren && isDefeatToast(defeatToastInChildren)) {
                  console.log('[Bestiary Automator] Defeat toast detected in child elements via MutationObserver for manual mode farming!');
                  handleDefeatInManualMode();
                  return; // Exit early since we found a defeat toast
                }
              }
    ...
    // Set up modal observer to watch for data-scroll-locked changes (only if manual mode farming is enabled)
    if (config.manualModeFarming) {
      setupModalObserver();
    }
  } catch (error) {
...
// Set up observer for openRewards state to detect when loot window opens
const setupModalObserver = () => {
  try {
    // Only set up observer if manual mode farming is enabled
    if (!config.manualModeFarming) {
      console.log('[Bestiary Automator] Manual mode farming disabled - skipping openRewards observer setup');
      return;
    }
    
    // Check if state.board exists
    if (!globalThis.state?.board?.select) {
      console.error('[Bestiary Automator] globalThis.state.board.select not available');
      return;
    }
    
    // Subscribe to openRewards state changes
    const openRewards = globalThis.state.board.select((ctx) => ctx.openRewards);
    openRewardsObserver = openRewards.subscribe((isOpen) => {
      // Only process if manual mode farming is still enabled
      if (!config.manualModeFarming) {
        return;
      }
      
      if (isOpen) {
        console.log('[Bestiary Automator] Loot rewards window opened (openRewards=true)');
        
        // Wait a bit for the actual game result to be processed by RunTracker
        setTimeout(() => {
          const boardContext = globalThis.state.board.getSnapshot().context;
          const gameTimerContext = globalThis.state.gameTimer.getSnapshot().context;
          const currentGameState = gameTimerContext?.state || boardContext?.state || 'initial';
          const defeatElements = document.querySelectorAll('h2.widget-top.widget-top-text, h2[class*="widget-top"]');
          let isDefeatWindow = false;

          // Check if we're in sandbox mode - if so, ignore this loot window
          if (isSandboxMode()) {
            console.log('[Bestiary Automator] Sandbox mode detected - ignoring loot window');
            return;
          }

          console.log('[Bestiary Automator] Current game state:', currentGameState);
          console.log('[Bestiary Automator] Board context state:', boardContext?.state);
          console.log('[Bestiary Automator] GameTimer context state:', gameTimerContext?.state);

          // Check server results for actual victory/defeat status (like RunTracker does)
          let actualVictory = null;
          let actualPoints = null;
          
          try {
            const serverResults = boardContext?.serverResults;
            if (serverResults && serverResults.rewardScreen) {
              actualVictory = serverResults.rewardScreen.victory;
              actualPoints = serverResults.rewardScreen.rank;
              console.log('[Bestiary Automator] Server results - victory:', actualVictory, 'points:', actualPoints);
            }
          } catch (error) {
            console.log('[Bestiary Automator] Could not get server results:', error);
          }

          // Determine if this is actually a victory or defeat based on server results
          if (actualVictory !== null) {
            if (actualVictory === true) {
              console.log('[Bestiary Automator] Victory confirmed by server results - NOT closing victory window');
              isDefeatWindow = false;
              // Stop manual mode farming on victory
              console.log('[Bestiary Automator] Victory detected - stopping manual mode farming');
              console.log('[Bestiary Automator] Resetting manualModeFarmingStarted flag - ready for next start button click');
              manualModeFarmingStarted = false;
              console.log('[Bestiary Automator] Flag reset complete. Click Start button to begin a new farming loop.');
            } else {
              console.log('[Bestiary Automator] Defeat confirmed by server results - will close loot window');
              isDefeatWindow = true;
            }
          } else if (actualPoints !== null && actualPoints === 0) {
            // 0 points = defeat (like RunTracker logic)
            console.log('[Bestiary Automator] Defeat confirmed by 0 points - will close loot window');
            isDefeatWindow = true;
          } else {
            // Fallback to game state checks
            if (currentGameState === 'defeat') {
              console.log('[Bestiary Automator] Defeat detected from game state - will close loot window');
              isDefeatWindow = true;
            } else if (boardContext && boardContext.gameStarted === false) {
              console.log('[Bestiary Automator] Game ended (gameStarted=false) - treating as defeat for manual mode farming');
              isDefeatWindow = true;
            } else {
              // Final fallback: Check DOM for defeat text
              for (const element of defeatElements) {
                const elementText = element.textContent || element.innerText || '';
                if (elementText.includes('Defeat') || elementText.includes('Derrota')) {
                  console.log('[Bestiary Automator] Defeat detected from DOM - will close loot window');
                  console.log('[Bestiary Automator] Defeat element text:', elementText);
                  isDefeatWindow = true;
                  break;
                }
              }
          ...
          if (isDefeatWindow) {
            // Close the loot window after a short delay to ensure it's fully loaded
            setTimeout(() => {
              closeLootSummaryWindow();
            }, 500);
          } else {
            console.log('[Bestiary Automator] Loot window opened but not defeat-related - leaving open');
          }
        }, 1500); // Wait 1.5 seconds for RunTracker to process the result
      } else {
        console.log('[Bestiary Automator] Loot rewards window closed (openRewards=false)');
      }
    });
    
    console.log('[Bestiary Automator] OpenRewards observer set up (manual mode farming only)');
  } catch (error) {
    console.error('[Bestiary Automator] Error setting up openRewards observer:', error);
  }
};

// Reconnect modal observer when settings change
const reconnectModalObserver = () => {
  // Disconnect existing observers
  if (modalObserver) {
    modalObserver.disconnect();
    modalObserver = null;
    console.log('[Bestiary Automator] Modal observer disconnected');
  }
  
  if (openRewardsObserver) {
    openRewardsObserver.unsubscribe();
    openRewardsObserver = null;
    console.log('[Bestiary Automator] OpenRewards observer unsubscribed');
  }
  
  // Set up new observer if manual mode farming is enabled
  if (config.manualModeFarming) {
    setupModalObserver();
  }
};
...
const processDefeatToast = async () => {
  ...
  // If manual mode farming is active, use the manual mode defeat handler
  if (config.manualModeFarming && manualModeFarmingStarted) {
    console.log('[Bestiary Automator] Defeat detected in manual mode farming - using manual mode handler');
    await handleDefeatInManualMode();
    setState(null);
    return true;
  }
  ...
  setTimeout(() => {
    setState(null);
    // Reset manual mode farming flag so user can configure again
    manualModeFarmingStarted = false;
    console.log('[Bestiary Automator] Manual mode farming flag reset - ready for next game');
    console.log('[Bestiary Automator] Defeat processing complete');
  }, 3000);
...
// Helper function to stop the game when victory is detected
const stopGameAfterVictory = async () => {
  try {
    console.log('[Bestiary Automator] Victory detected - stopping game...');
    
    // First try to pause autoplay using the pause button (like Better Tasker)
    const pauseButton = await findPauseButton();
    if (pauseButton) {
      console.log('[Bestiary Automator] Found pause button - clicking to stop autoplay...');
      
      // Clear any modals by simulating ESC key presses
      await clearModalsWithEsc(3);
      await sleep(100);
      
      pauseButton.click();
      await sleep(300);
      console.log('[Bestiary Automator] Pause button clicked successfully');
    } else {
      console.log('[Bestiary Automator] Pause button not found - using state manipulation fallback');
      
      // Fallback: Use direct state manipulation to stop the game
      if (globalThis.state && globalThis.state.board) {
        ...
        console.log('[Bestiary Automator] Game stopped via state manipulation');
      }
    }
    ...
    console.error('[Bestiary Automator] Error stopping game after victory:', error);
  }
};

// Helper function to find pause button (based on Better Tasker implementation)
const findPauseButton = async () => {
  try {
    console.log('[Bestiary Automator] Looking for pause button...');
    
    // Define selectors for pause button (based on Better Tasker)
    const selectors = [
    ...
    if (!button) {
      ...
      if (flexContainer) {
        ...
        if (buttons.length >= 2) {
          ...
          if (hasPauseIcon) {
            button = secondButton;
            console.log('[Bestiary Automator] Found pause button using structure fallback');
          }
        }
      }
    }
    ...
    if (button) {
      console.log('[Bestiary Automator] Pause button found');
      return button;
    } else {
      console.log('[Bestiary Automator] Pause button not found');
      return null;
    }
  } catch (error) {
    console.error('[Bestiary Automator] Error finding pause button:', error);
    return null;
  }
};

// Helper function to clear modals with ESC key presses (based on Better Tasker)
const clearModalsWithEsc = async (count = 3) => {
  try {
    console.log(`[Bestiary Automator] Clearing modals with ${count} ESC key presses...`);
    ...
    console.log('[Bestiary Automator] ESC key presses completed');
  } catch (error) {
    console.error('[Bestiary Automator] Error clearing modals with ESC:', error);
  }
};

// Helper function to switch to manual mode for farming
const switchToManualMode = () => {
  try {
    console.log('[Bestiary Automator] Switching to manual mode for farming...');
    
    if (globalThis.state && globalThis.state.board) {
      globalThis.state.board.send({ type: "setPlayMode", mode: "manual" });
      console.log('[Bestiary Automator] Switched to manual mode');
      return true;
    }
    return false;
  } catch (error) {
    console.error('[Bestiary Automator] Error switching to manual mode:', error);
    return false;
  }
};

// Helper function to find skip button (appears when time limit is exceeded)
const findSkipButton = () => {
  try {
    // First, try the specific selector you provided
    ...
    if (specificSkipButton) {
      ...
      if (buttonText.includes('Skip') || buttonText.includes('Pular')) {
        console.log('[Bestiary Automator] Found skip button using specific selector');
        console.log('[Bestiary Automator] Button text:', `"${buttonText}"`);
    ...
    for (const button of skipButtons) {
      const buttonText = button.textContent.trim();
      if (buttonText.includes('Skip') || buttonText.includes('Pular')) {
        console.log('[Bestiary Automator] Found skip button using text search');
        console.log('[Bestiary Automator] Button text:', `"${buttonText}"`);
        return button;
      }
    }
    
    // Additional fallback: Look for buttons with data attributes (old method)
    ...
    for (const button of skipButtonsByAttr) {
      const buttonText = button.textContent.trim();
      if (buttonText.includes('Skip') || buttonText.includes('Pular')) {
        console.log('[Bestiary Automator] Found skip button using data attributes');
        console.log('[Bestiary Automator] Button text:', `"${buttonText}"`);
        return button;
      }
    }
    ...
    console.error('[Bestiary Automator] Error finding skip button:', error);
    return null;
  }
};

// Helper function to handle skip button click
const handleSkipButton = async () => {
  try {
    const skipButton = findSkipButton();
    if (skipButton) {
      console.log('[Bestiary Automator] Skip button found - clicking to skip time-limit loss...');
      skipButton.click();
      await sleep(500); // Wait for skip to process
      console.log('[Bestiary Automator] Skip button clicked successfully');
    ...
    console.error('[Bestiary Automator] Error handling skip button:', error);
    return false;
  }
};

// Helper function to handle defeat in manual mode farming
const handleDefeatInManualMode = async () => {
  try {
    // Skip if in sandbox mode
    if (isSandboxMode()) {
      console.log('[Bestiary Automator] Defeat detected in sandbox mode - ignoring');
      return;
    }
    
    console.log('[Bestiary Automator] Defeat detected in manual mode farming - stopping game...');
    
    // Use direct state manipulation to stop the game completely
    if (globalThis.state && globalThis.state.board) {
      globalThis.state.board.send({
        type: "setState",
        fn: prevState => ({
      ...
      console.log('[Bestiary Automator] Game stopped after defeat');
    }
    
    // Click stop button to stop the game (backup method)
    const stopButton = findStopButton();
    if (stopButton) {
      stopButton.click();
      console.log('[Bestiary Automator] Stop button clicked after defeat');
      await sleep(500);
    }
    
    // Close loot summary window - try multiple times with delays
    setTimeout(() => {
      closeLootSummaryWindow();
    }, 1000);
    
    // Try again after 2 seconds in case the window takes longer to appear
    setTimeout(() => {
      closeLootSummaryWindow();
    }, 2000);
    
    // Try again after 3 seconds as final attempt
    setTimeout(() => {
      closeLootSummaryWindow();
    }, 3000);
    ...
    // Reset manual mode farming flag so user can configure again
    manualModeFarmingStarted = false;
    console.log('[Bestiary Automator] Manual mode farming flag reset after defeat');
    
    return true;
  } catch (error) {
    console.error('[Bestiary Automator] Error handling defeat in manual mode:', error);
    return false;
  }
};

// Helper function to handle victory in manual mode farming
const handleVictoryInManualMode = async () => {
  try {
    // Skip if in sandbox mode
    if (isSandboxMode()) {
      console.log('[Bestiary Automator] Victory detected in sandbox mode - ignoring');
    ...
    console.log('[Bestiary Automator] Victory detected in manual mode farming - stopping game...');
    ...
    if (globalThis.state && globalThis.state.board) {
      ...
      console.log('[Bestiary Automator] Game stopped after victory');
    }
    ...
    if (stopButton) {
      stopButton.click();
      console.log('[Bestiary Automator] Stop button clicked after victory');
      await sleep(500);
    }
    ...
    // Reset manual mode farming flag so user can configure again
    manualModeFarmingStarted = false;
    console.log('[Bestiary Automator] Manual mode farming flag reset after victory');
    
    return true;
  } catch (error) {
    console.error('[Bestiary Automator] Error handling victory in manual mode:', error);
    return false;
  }
};

// Main manual mode farming function
const handleManualModeFarming = async () => {
  if (!config.manualModeFarming) return;
  
  try {
    // Check if we're in sandbox mode - if so, skip manual mode farming
    if (isSandboxMode()) {
      console.log('[Bestiary Automator] Sandbox mode detected - skipping manual mode farming');
      return;
    }
    
    // Check if manual mode farming has been started by user clicking start
    if (!manualModeFarmingStarted) {
      console.log('[Bestiary Automator] Manual mode farming not started yet - waiting for user to click start first');
      return;
    }
    
    // Check if other mods are controlling autoplay - if so, skip manual mode farming
    if (window.AutoplayManager && window.AutoplayManager.getCurrentOwner() && 
        window.AutoplayManager.getCurrentOwner() !== 'Bestiary Automator') {
      console.log('[Bestiary Automator] Another mod is controlling autoplay - skipping manual mode farming');
      return;
    }
    
    // PRIORITY: Check for skip button FIRST (time-limit loss)
    // This is the most important check and should happen before anything else
    const skipButton = findSkipButton();
    if (skipButton) {
      console.log('[Bestiary Automator] Skip button detected in manual mode farming - clicking immediately');
      await handleSkipButton();
      return;
    }
    
    // PRIORITY: Check for defeat toast SECOND (defeat detection)
    // This should happen before other checks
    const defeatToasts = document.querySelectorAll('div.widget-bottom.pixel-font-16.flex.items-center.gap-2.px-2.py-1.text-whiteHighlight:has(img[alt="no"])');
    for (const toast of defeatToasts) {
      if (isDefeatToast(toast)) {
        console.log('[Bestiary Automator] Defeat toast detected in manual mode farming - handling defeat');
        await handleDefeatInManualMode();
        return;
      }
    }
    ...
    const boardContext = globalThis.state?.board?.getSnapshot()?.context;
    if (boardContext?.mode !== 'manual') {
      console.log('[Bestiary Automator] Not in manual mode - switching to manual mode for farming');
      switchToManualMode();
      return;
    }
    
    // Check if game is not started and we need to start it
    // Only start if manual mode farming is still active (flag not reset)
    if (!boardContext?.gameStarted && manualModeFarmingStarted) {
      const startButton = findStartButton();
      if (startButton) {
        console.log('[Bestiary Automator] Starting new game in manual mode');
        startButton.click();
        await sleep(500);
      }
    } else if (!boardContext?.gameStarted && !manualModeFarmingStarted) {
      // Manual mode farming flag was reset, don't start game automatically
      console.log('[Bestiary Automator] Game not started and manual mode farming flag reset - waiting for user to click start');
    }
    
  } catch (error) {
    console.error('[Bestiary Automator] Error in manual mode farming:', error);
  }
};
...
const findStopButton = () => {
  try {
    // First, try the specific selector (same location as skip button)
    const specificStopButton = document.querySelector("#__next > div.flex.min-h-screen.flex-col > div > main > div > div.flex.flex-col.justify-center.gap-2.\\32 xl\\:flex-row > div.grid.grid-cols-2.gap-2.gap-y-1\\.5.sm\\:grid-cols-\\[1fr_min-content\\].sm\\:grid-rows-\\[3fr\\].sm\\:gap-y-2.\\32 xl\\:grid-cols-\\[min-content\\] > div.sm\\:order-3 > button");
    if (specificStopButton) {
      const buttonText = specificStopButton.textContent.trim();
      if (buttonText.includes('Stop') || buttonText.includes('Parar')) {
        console.log('[Bestiary Automator] Found stop button using specific selector');
        console.log('[Bestiary Automator] Button text:', `"${buttonText}"`);
        return specificStopButton;
      }
    }
    
    // Fallback: Look for stop button by text content in all buttons
    const stopButtons = document.querySelectorAll('button');
    for (const button of stopButtons) {
      const buttonText = button.textContent.trim();
      if (buttonText.includes('Stop') || buttonText.includes('Parar')) {
        console.log('[Bestiary Automator] Found stop button using text search');
        console.log('[Bestiary Automator] Button text:', `"${buttonText}"`);
        return button;
      }
    }
    ...
    // Additional fallback: Look for buttons with data attributes (old method)
    const stopButtonsByAttr = document.querySelectorAll('button[data-full="false"][data-state="closed"]');
    for (const button of stopButtonsByAttr) {
      const buttonText = button.textContent.trim();
      if (buttonText.includes('Stop') || buttonText.includes('Parar')) {
        console.log('[Bestiary Automator] Found stop button using data attributes');
        console.log('[Bestiary Automator] Button text:', `"${buttonText}"`);
        return button;
      }
    }
    ...
    console.error('[Bestiary Automator] Error finding stop button:', error);
    return null;
  }
};

// Helper function to close loot summary window using ESC key
const closeLootSummaryWindow = () => {
  try {
    console.log('[Bestiary Automator] Attempting to close loot summary window using ESC key...');
    
    // Check if there's a modal open (data-scroll-locked="1")
    const bodyElement = document.body;
    if (bodyElement.getAttribute('data-scroll-locked') === '1') {
      console.log('[Bestiary Automator] Modal detected (data-scroll-locked="1") - pressing ESC to close...');
      ...
      document.dispatchEvent(escEvent);
      console.log('[Bestiary Automator] ESC key pressed to close modal');
      return true;
    }
    
    console.log('[Bestiary Automator] No modal detected (data-scroll-locked not set to "1")');
    return false;
  } catch (error) {
    console.error('[Bestiary Automator] Error closing loot summary window:', error);
    return false;
  }
};

// Track if start button click listener has been set up
let startButtonClickListenerSetup = false;
let startButtonClickHandler = null;

// Helper function to detect when user clicks start button for manual mode farming
const detectStartButtonClick = () => {
  if (!config.manualModeFarming) {
    // Remove listener if manual mode farming is disabled
    if (startButtonClickListenerSetup && startButtonClickHandler) {
      document.removeEventListener('click', startButtonClickHandler, true);
      startButtonClickListenerSetup = false;
      startButtonClickHandler = null;
      console.log('[Bestiary Automator] Start button click listener removed (manual mode farming disabled)');
    }
    return;
  }
  
  // Set up event delegation on document to catch all Start button clicks
  // This is more robust because it works even if buttons are recreated
  // The listener stays active so it can detect clicks even after victory/defeat reset the flag
  if (!startButtonClickListenerSetup) {
    startButtonClickHandler = (event) => {
      // Check if manual mode farming is enabled
      if (!config.manualModeFarming) {
        return;
      }
      
      // Check if flag is already set (already started) - if so, ignore
      if (manualModeFarmingStarted) {
        return;
      }
      ...
      const buttonText = button.textContent.trim();
      const isStartButton = buttonText === 'Start' || buttonText === 'Iniciar';
      
      if (isStartButton) {
        console.log('[Bestiary Automator] User clicked start button - manual mode farming now active');
        console.log('[Bestiary Automator] Flag was:', manualModeFarmingStarted, '-> now setting to true');
        manualModeFarmingStarted = true;
      }
    };
    
    // Use capture phase to catch clicks early
    document.addEventListener('click', startButtonClickHandler, true);
    startButtonClickListenerSetup = true;
    
    console.log('[Bestiary Automator] Start button click listener set up using event delegation (stays active)');
  }
};
...
// Helper function to check if an element is a skip button
const isSkipButton = (element) => {
  try {
    // Check if the element is a button
    if (element.tagName !== 'BUTTON') return false;
    
    // Check the button text - this is the most reliable way
    const buttonText = element.textContent.trim();
    const isSkipText = buttonText.includes('Skip') || buttonText.includes('Pular');
    
    if (isSkipText) {
      console.log('[Bestiary Automator] Skip button detected in MutationObserver:', element);
      console.log('[Bestiary Automator] Button text:', `"${buttonText}"`);
      return true;
    }
    
    // Also check if the button matches the specific selector path structure
    // Check if it's inside a div with class "sm:order-3" (part of the selector you provided)
    const parentDiv = element.closest('div.sm\\:order-3');
    if (parentDiv && isSkipText) {
      console.log('[Bestiary Automator] Skip button detected in specific structure');
      return true;
    }
    
    return false;
  } catch (error) {
    console.error('[Bestiary Automator] Error checking if element is skip button:', error);
    return false;
  }
};
...
const runAutomationTasks = async () => {
  try {
    ...
    // Check if we're in sandbox mode - if so, skip all automation tasks
    if (isSandboxMode()) {
      console.log('[Bestiary Automator] Sandbox mode detected, skipping all automation tasks');
      return;
    }
    ...
    // Detect start button click for manual mode farming
    detectStartButtonClick();
    
    // Run manual mode farming if enabled
    await handleManualModeFarming();
...
const createConfigPanel = () => {
  ...
  // Stop after victory checkbox
  const stopAfterVictoryContainer = createCheckboxContainer('stop-after-victory-checkbox', t('mods.automator.stopAfterVictory'), config.stopAfterVictory);
  
  // Manual mode farming checkbox
  const manualModeFarmingContainer = createCheckboxContainer('manual-mode-farming-checkbox', t('mods.automator.manualModeFarming'), config.manualModeFarming);
  ...
  content.appendChild(stopAfterVictoryContainer);
  content.appendChild(manualModeFarmingContainer);
  ...
  setTimeout(() => {
    ...
    const stopAfterVictoryCheckbox = document.getElementById('stop-after-victory-checkbox');
    const manualModeFarmingCheckbox = document.getElementById('manual-mode-farming-checkbox');
    ...
    if (stopAfterVictoryCheckbox) stopAfterVictoryCheckbox.checked = config.stopAfterVictory;
    if (manualModeFarmingCheckbox) manualModeFarmingCheckbox.checked = config.manualModeFarming;
    ...
    buttons: [
      {
        ...
        onClick: () => {
          ...
          config.stopAfterVictory = document.getElementById('stop-after-victory-checkbox').checked;
          config.manualModeFarming = document.getElementById('manual-mode-farming-checkbox').checked;
          ...
          const configToSave = {
            ...
            stopAfterVictory: config.stopAfterVictory,
            manualModeFarming: config.manualModeFarming,
          ...
          // Reconnect modal observer if manual mode farming setting changed
          reconnectModalObserver();
          ...
          // Update game state subscription based on autoplay, victory, and manual mode settings
          if (config.autoPlayAfterDefeat || config.stopAfterVictory || config.manualModeFarming) {
            subscribeToGameState();
          } else {
            unsubscribeFromGameState();
          }
          
          // Reset manual mode farming flag if disabled
          if (!config.manualModeFarming) {
            manualModeFarmingStarted = false;
            console.log('[Bestiary Automator] Manual mode farming disabled - flag reset');
          }
...
const applyButtonStyling = (btn) => {
  ...
  
  {
    ...
    console.log('  - stopAfterVictory:', config.stopAfterVictory);
    console.log('  - manualModeFarming:', config.manualModeFarming);
  ...
  if (config.autoRefillStamina) {
    ...
    btn.style.backgroundSize = "auto";
  } else if (config.autoCollectRewards || config.autoDayCare || config.autoPlayAfterDefeat || config.stopAfterVictory || config.manualModeFarming) {
...
function init() {
  ...
  // Subscribe to game state if autoplay after defeat, stop after victory, or manual mode farming is enabled
  if (config.autoPlayAfterDefeat || config.stopAfterVictory || config.manualModeFarming) {
    subscribeToGameState();
  }
  
  // Reset manual mode farming flag on initialization
  manualModeFarmingStarted = false;
  console.log('[Bestiary Automator] Manual mode farming flag reset on initialization');
...
function updateSettingsModalUI() {
  try {
    ...
    const stopAfterVictoryCheckbox = document.getElementById('stop-after-victory-checkbox') || 
                                   document.querySelector('input[type="checkbox"][id*="victory"]');
    
    const manualModeFarmingCheckbox = document.getElementById('manual-mode-farming-checkbox') || 
                                    document.querySelector('input[type="checkbox"][id*="manual"]');
...
context.exports = {
  ...
  cleanup: () => {
    ...
    // Remove start button click listener
    if (startButtonClickListenerSetup && startButtonClickHandler) {
      document.removeEventListener('click', startButtonClickHandler, true);
      startButtonClickListenerSetup = false;
      startButtonClickHandler = null;
      console.log('[Bestiary Automator] Start button click listener removed during cleanup');
    }
