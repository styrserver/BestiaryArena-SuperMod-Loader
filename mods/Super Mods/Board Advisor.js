...
async function initSandboxDB() {
  return new Promise((resolve, reject) => {
    ...
    const request = indexedDB.open(SANDBOX_DB_NAME);
...
class DataCollector {
  ...
  storeRunData(runData) {
    ...
    if (runData.completed === false) {
      console.log(`[Board Advisor] Skipping failed run for ${runData.mapName || runData.roomId}`);
      return; // Exit early without storing the data
    }
...
addWindowListener('message', (event) => {
  if (event.data && event.data.message && event.data.message.action === 'updateLocalModState') {
    ...
    if (modName === 'Super Mods/Board Advisor.js' && !enabled) {
