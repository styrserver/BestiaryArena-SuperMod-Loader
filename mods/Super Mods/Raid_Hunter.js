...
function isBetterTaskerProcessingTask() {
    try {
        ...
        if (typeof window !== 'undefined' && window.betterTaskerState) {
            ...
            return taskerState.taskOperationInProgress || taskerState.taskHuntingOngoing || taskerState.pendingTaskCompletion;
        }
...
function cleanupRaidHunter(periodic = false) {
    try {
        // Clean up modal if open only if not periodic cleanup
        if (!periodic && activeRaidHunterModal) {
            cleanupRaidHunterModal();
        }
