...
const defaultConfig = {
  ...
  currentLocale: 'en' // Always use English
};

// Initialize with saved config or defaults
let savedConfig = {};
try {
  savedConfig = api.service.getScriptConfig
    ? api.service.getScriptConfig('simulator-config') || {}
    : {};
} catch (e) { savedConfig = {}; }
const config = Object.assign({}, defaultConfig, savedConfig);

// Constants
const MOD_ID = 'simulator';
...
const REWARD_CONFIG = {
  baseStamina: 10, // Base stamina cost per run
  drops: {
    gold: {
      type: 'resource',
      rate: 12.58, // per stamina
      name: 'Gold'
    },
    dust: {
      type: 'resource',
      rate: 0.07, // per stamina
      name: 'Dust'
    },
    equipment: {
      ad: { rate: 0.10, name: 'AD Equipment' },
      ap: { rate: 0.10, name: 'AP Equipment' },
      hp: { rate: 0.10, name: 'HP Equipment' }
    },
    diceManipulators: {
      common: { rate: 9.51, name: 'Common Dice' },
      uncommon: { rate: 0.65, name: 'Uncommon Dice' },
      rare: { rate: 0.59, name: 'Rare Dice' },
      mythic: { rate: 0.31, name: 'Mythic Dice' },
      legendary: { rate: 0.05, name: 'Legendary Dice' }
    },
    staminaPotions: {
      mini: { rate: 1.33, name: 'Mini Potion' },
      strong: { rate: 0.76, name: 'Strong Potion' },
      great: { rate: 0.21, name: 'Great Potion' },
      ultimate: { rate: 0.17, name: 'Ultimate Potion' }
    },
    summonScrolls: {
      crude: { rate: 1.85, name: 'Crude Scroll' },
      ordinary: { rate: 0.36, name: 'Ordinary Scroll' },
      refined: { rate: 0.21, name: 'Refined Scroll' },
      special: { rate: 0.13, name: 'Special Scroll' },
      exceptional: { rate: 0.04, name: 'Exceptional Scroll' }
    },
    creatures: {
      grey: { rate: 38.25, name: 'Grey Creature' },
      green: { rate: 29.91, name: 'Green Creature' },
      blue: { rate: 22.44, name: 'Blue Creature' },
      purple: { rate: 7.90, name: 'Purple Creature' },
      yellow: { rate: 1.50, name: 'Yellow Creature' }
    }
  }
};

// Map name to stamina cost lookup
const MAP_STAMINA_COSTS = {
  "Sewers": 3,
  "Wheat Field": 3,
  "Evergreen Fields": 6,
  "Wolf's Den": 6,
  ...
  "Carlin Sewers": 6,
  "Isle of Kings": 6,
  "Ghostlands Surface": 9,
  "Ghostlands Library": 9,
  "Ghostlands Ritual Site": 9,
  "Demon Skeleton Hell": 9,
  "Zathroth's Throne": 9,
  "Demonrage Seal": 12,
  "Banshee's Last Room": 12,
  "Maze Gates": 9,
  "Labyrinth Depths": 9,
  "Hidden City of Demona": 9,
  "Teleporter Trap": 18,
  "Folda Boat": 9,
  "Cave Entrance": 6,
  "Frozen Aquifer": 6,
  ...
  "Femor Hills": 9,
  "Orcish Barracks": 12,
  "The Orc King Hall": 15,
  "A Shamanic Ritual": 15,
  "Shore Camp": 9,
  "Orcsmith Orchshop": 6,
  "Orc Fortress Outskirts": 9,
  "The Farms": 9,
  "Dwarven Brewery": 9,
  "Mine Hub": 15,
  "Emperor Kruzak's Treasure Room": 9,
  "Mad Technomancer's Lab": 18,
  "Awash Steamship": 6,
  "Robson's Isle Ruins": 9
};

// Helper to get stamina cost for the current map name
function getStaminaCostForCurrentMap(mapName) {
  if (mapName && MAP_STAMINA_COSTS.hasOwnProperty(mapName)) {
    return MAP_STAMINA_COSTS[mapName];
  }
...
const TRANSLATIONS = {
  en: {
    ...
    buttonTooltip: 'Simulate current board performance',
    configButtonTooltip: 'Simulator Settings',
    configTitle: 'Board Simulator Settings',
    ...
    // New reward-related translations
    ...
    diceManipulatorsLabel: 'Dice Manipulators',
    staminaPotionsLabel: 'Stamina Potions',
    summonScrollsLabel: 'Summon Scrolls',
...
function createConfigPanel(startAnalysisCallback) {
  ...
  function updateConfig() {
    ...
    // Save configuration
    api.service.updateScriptConfig('simulator-config', config);
  }
...
function showResultsModal(results) {
  ...
  setTimeout(() => {
    ...
    const rewards = calculateAverageRewards(results.results);

    // --- Rewards Section ---
    const rewardsSection = document.createElement('div');
    rewardsSection.style.cssText = 'margin-top: 10px; border-top: 1px solid #333; padding-top: 10px;';

    // Rewards Title
    const rewardsTitle = document.createElement('div');
    rewardsTitle.textContent = t('rewardsTitle');
    rewardsTitle.style.cssText = 'margin-bottom: 10px; color: #3498db; font-weight: bold; font-size: 1.1em;';
    rewardsSection.appendChild(rewardsTitle);

    // --- Resources ---
    const resourcesSection = document.createElement('div');
    resourcesSection.style.cssText = 'margin-bottom: 10px;';
    const resourcesTitle = document.createElement('div');
    resourcesTitle.textContent = t('resourcesTitle');
    resourcesTitle.style.cssText = 'margin-bottom: 4px; color: #2ecc71; font-weight: bold;';
    resourcesSection.appendChild(resourcesTitle);

    // Resources grid (single column)
    const resourcesGrid = document.createElement('div');
    resourcesGrid.style.cssText = 'display: grid; grid-template-columns: 1fr 60px; gap: 2px; font-size: 1em;';

    // Gold
    const goldLabel = document.createElement('div');
    goldLabel.textContent = t('goldLabel');
    const goldValue = document.createElement('div');
    goldValue.textContent = `${rewards.total.resources.gold}`;
    goldValue.style.cssText = 'text-align: right; color: gold; font-weight: bold;';
    resourcesGrid.appendChild(goldLabel);
    resourcesGrid.appendChild(goldValue);

    // Dust
    const dustLabel = document.createElement('div');
    dustLabel.textContent = t('dustLabel');
    const dustValue = document.createElement('div');
    dustValue.textContent = `${rewards.total.resources.dust}`;
    dustValue.style.cssText = 'text-align: right; color: #3498db; font-weight: bold;';
    resourcesGrid.appendChild(dustLabel);
    resourcesGrid.appendChild(dustValue);

    resourcesSection.appendChild(resourcesGrid);
    rewardsSection.appendChild(resourcesSection);

    // --- Items ---
    const itemsSection = document.createElement('div');
    const itemsTitle = document.createElement('div');
    itemsTitle.textContent = t('itemsTitle');
    itemsTitle.style.cssText = 'margin-bottom: 4px; color: #2ecc71; font-weight: bold;';
    itemsSection.appendChild(itemsTitle);

    // Helper to create item category
    function createItemCategory(category, label) {
      const categorySection = document.createElement('div');
      categorySection.style.cssText = 'margin-bottom: 6px;';
      const categoryTitle = document.createElement('div');
      categoryTitle.textContent = label;
      categoryTitle.style.cssText = 'margin-bottom: 2px; color: #e67e22; font-weight: bold; font-size: 1em;';
      categorySection.appendChild(categoryTitle);
      const itemsGrid = document.createElement('div');
      itemsGrid.style.cssText = 'display: grid; grid-template-columns: 1fr 40px; gap: 1px; font-size: 0.97em;';
      for (const [item, count] of Object.entries(rewards.total.items[category])) {
        const itemLabel = document.createElement('div');
        itemLabel.textContent = REWARD_CONFIG.drops[category][item].name;
        const itemValue = document.createElement('div');
        itemValue.textContent = `${parseInt(count, 10)}`;
        itemValue.style.cssText = 'text-align: right;';
        itemsGrid.appendChild(itemLabel);
        itemsGrid.appendChild(itemValue);
      }
      categorySection.appendChild(itemsGrid);
      return categorySection;
    }
    itemsSection.appendChild(createItemCategory('equipment', t('equipmentLabel')));
    itemsSection.appendChild(createItemCategory('diceManipulators', t('diceManipulatorsLabel')));
    itemsSection.appendChild(createItemCategory('staminaPotions', t('staminaPotionsLabel')));
    itemsSection.appendChild(createItemCategory('summonScrolls', t('summonScrollsLabel')));
    itemsSection.appendChild(createItemCategory('creatures', t('creaturesLabel')));
    rewardsSection.appendChild(itemsSection);
...
// Function to calculate rewards for a single run
function calculateRunRewards(ticks, mapName) {
  const rewards = {
    resources: {},
    items: []
  };
  
  // Use the actual stamina cost for the map
  const staminaUsed = getStaminaCostForCurrentMap(mapName);
  
  // Calculate resource rewards
  rewards.resources.gold = Math.floor(REWARD_CONFIG.drops.gold.rate * staminaUsed);
  rewards.resources.dust = Math.floor(REWARD_CONFIG.drops.dust.rate * staminaUsed);
  
  // Helper function to check for drops
  function checkDrop(category, item) {
    const dropRate = REWARD_CONFIG.drops[category][item].rate;
    for (let i = 0; i < staminaUsed; i++) {
      const roll = Math.random() * 100;
      if (roll < dropRate) {
        rewards.items.push({
          type: category,
          name: REWARD_CONFIG.drops[category][item].name,
          rarity: item
        });
      }
    }
  ...
  for (const type of ['ad', 'ap', 'hp']) {
    checkDrop('equipment', type);
  }
  
  // Check for dice manipulator drops
  for (const rarity of ['common', 'uncommon', 'rare', 'mythic', 'legendary']) {
    checkDrop('diceManipulators', rarity);
  }
  
  // Check for stamina potion drops
  for (const type of ['mini', 'strong', 'great', 'ultimate']) {
    checkDrop('staminaPotions', type);
  }
  
  // Check for summon scroll drops
  for (const type of ['crude', 'ordinary', 'refined', 'special', 'exceptional']) {
    checkDrop('summonScrolls', type);
  }
  
  // Always drop exactly one creature per run, regardless of stamina cost
  const creatureRoll = Math.random() * 100;
  let cumulativeRate = 0;
  let selectedTier = 'grey';
  for (const tier of ['grey', 'green', 'blue', 'purple', 'yellow']) {
    cumulativeRate += REWARD_CONFIG.drops.creatures[tier].rate;
    if (creatureRoll < cumulativeRate) {
      selectedTier = tier;
      break;
    }
  }
  rewards.items.push({
    type: 'creatures',
    name: REWARD_CONFIG.drops.creatures[selectedTier].name,
    rarity: selectedTier
  });
...
// Function to calculate average rewards across multiple runs
function calculateAverageRewards(results) {
  const totalRewards = {
    resources: {
      gold: 0,
      dust: 0
  ...
  // Initialize counters for all possible items
  for (const category in REWARD_CONFIG.drops) {
    if (category !== 'gold' && category !== 'dust') {
      for (const item in REWARD_CONFIG.drops[category]) {
        totalRewards.items[category][item] = 0;
      }
    }
  }
  
  // Sum up rewards from all runs
  results.forEach(result => {
    // Try to get map name from result, fallback to currentRoomName
    const mapName = result.mapName || currentRoomName;
    const runRewards = calculateRunRewards(result.ticks, mapName);
    
    // Add resources
    totalRewards.resources.gold += runRewards.resources.gold;
    totalRewards.resources.dust += runRewards.resources.dust;
    
    // Add items
    runRewards.items.forEach(item => {
      totalRewards.items[item.type][item.rarity]++;
    });
  });
  ...
  const averages = {
    resources: {
      gold: Math.floor(totalRewards.resources.gold / numRuns),
      dust: Math.floor(totalRewards.resources.dust / numRuns)
    },
    items: {}
  };
  for (const category in totalRewards.items) {
    averages.items[category] = {};
    for (const item in totalRewards.items[category]) {
      averages.items[category][item] = (totalRewards.items[category][item] / numRuns).toFixed(2);
    }
  }
  return {
    total: totalRewards,
