...
(function() {
  ...
  // Lock (or unlock) the parts of the UI that must stay put while a forge runs:
  // the Disenchant/Auto-upgrade tab buttons and the equipment/tier/stat pickers.
  function setForgeUiLocked(locked) {
    try {
      ['better-forge-tab-disenchant', 'better-forge-tab-upgrade'].forEach(id => {
      ...
      const autoUpgradeRoot = document.getElementById('auto-upgrade-content');
      if (autoUpgradeRoot) {
        autoUpgradeRoot.classList.toggle('forge-locked', !!locked);
      }
    } catch (error) {
      console.warn('[Better Forge] Error toggling forge UI lock:', error);
    }
  }
  ...
  function clearAutoUpgradeVisualSelection() {
    ...
    // Unlock the search bar (it was disabled + filled while an equipment was selected)
    setSearchInputLockedToEquipment(null);
  ...
  // Lock the Arsenal (col1) search bar to a selected auto-upgrade equipment.
  // Passing a name fills the field and disables it (greyed out); passing a
  // falsy value re-enables and clears it.
  function setSearchInputLockedToEquipment(equipmentName, inputEl) {
    try {
      const searchInput = inputEl || document.getElementById('better-forge-search');
      if (!searchInput) return;

      if (equipmentName) {
        // Selecting an equipment discards any prior free-text search ("wh" etc.)
        forgeState.globalSearchTerm = '';
        ...
        // Restore any free-text search term the user had typed before selecting
        const restored = forgeState.globalSearchTerm || '';
        if (searchInput.value !== restored) {
          searchInput.value = restored;
        }
      }
    } catch (error) {
      console.warn('[Better Forge] Error locking search input to equipment:', error);
    }
  }

  // Scroll a named item in the auto-upgrade equipment list into view, moving only
  // that list's own scrollbar (not the modal / page).
  function scrollAutoUpgradeItemIntoView(equipmentName) {
    try {
      if (!equipmentName) return;
      const col = document.getElementById('auto-upgrade-equipment-col');
      if (!col) return;

      const target = equipmentName.toLowerCase();
      const item = Array.from(col.querySelectorAll('.auto-upgrade-item')).find(el =>
        (el.getAttribute('title') || el.textContent || '').toLowerCase() === target
      );
      if (!item) return;

      const colRect = col.getBoundingClientRect();
      const itemRect = item.getBoundingClientRect();
      col.scrollTop += (itemRect.top - colRect.top) - (col.clientHeight - item.clientHeight) / 2;
    } catch (error) {
      console.warn('[Better Forge] Error scrolling auto-upgrade item into view:', error);
    }
  }
  ...
  function filterCol1BySelectedEquipment(selectedEquipment) {
    try {
      // Remember what was selected so we can restore the list's scroll position on deselect
      const previousEquipment = forgeState.selectedEquipmentFilter;
      ...
      // Reflect the selection in the search bar (fill + disable, or clear + enable)
      setSearchInputLockedToEquipment(selectedEquipment);

      // Auto-upgrade equipment list: show only the selected item, or all when deselected
      if (selectedEquipment) {
        filterAutoUpgradeEquipmentList(selectedEquipment, null, true);
      } else {
        filterAutoUpgradeEquipmentList('');
        // Keep the just-deselected item in view instead of snapping back to the top
        scrollAutoUpgradeItemIntoView(previousEquipment);
      }
  ...
  // Filter the auto-upgrade tab's "Equipment" list to match the Arsenal (col1) search term.
  // Purely a show/hide pass over the existing .auto-upgrade-item nodes.
  function filterAutoUpgradeEquipmentList(searchTerm, container, exact) {
    try {
      const col = container || document.getElementById('auto-upgrade-equipment-col');
      if (!col) return;

      const term = (searchTerm || '').toString().toLowerCase().trim();
      const items = col.querySelectorAll('.auto-upgrade-item');
      let visibleCount = 0;

      items.forEach(item => {
        const name = (item.getAttribute('title') || item.textContent || '').toLowerCase();
        const isMatch = !term || (exact ? name === term : name.includes(term));
        item.style.display = isMatch ? '' : 'none';
        if (isMatch) visibleCount++;
      });

      // Manage a lightweight "no results" placeholder
      let emptyMsg = col.querySelector('.auto-upgrade-equipment-empty');
      if (term && visibleCount === 0) {
        if (!emptyMsg) {
          emptyMsg = document.createElement('div');
          emptyMsg.className = 'auto-upgrade-equipment-empty';
      ...
      console.warn('[Better Forge] Error filtering auto-upgrade equipment list:', error);
    }
  }
  ...
  function handleBetterForgeModalClose() {
       ...
       try {
         ...
         handleError(error, 'showBetterForgeModal', true);
          ...
          updateFilterButtonText(filterBtn, currentFilter);
                   ...
                   const selectedEquipment = forgeState.selectedEquipmentFilter || getCurrentSelection().selectedEquipment;
                   if (selectedEquipment) {
                     // Override with equipment filter if selected — lock the field (filled + disabled)
                     setSearchInputLockedToEquipment(selectedEquipment, searchInput);
                   ...
                   const debouncedSearch = (value) => {
                     ...
                     searchTimeout = setTimeout(() => {
                       ...
                       // Mirror the search onto the auto-upgrade tab's equipment list
                       filterAutoUpgradeEquipmentList(value);
                   ...
                   eventManager.add(scrollArea, 'searchCleared', () => {
                     // Restore the auto-upgrade tab's equipment list when the search is cleared
                     filterAutoUpgradeEquipmentList('');
   ...
   function getAutoUpgradeContent() {
    try {
      ...
      // Carry over an active Arsenal (col1) search term to this list
      if (forgeState.globalSearchTerm) {
        filterAutoUpgradeEquipmentList(forgeState.globalSearchTerm, equipmentContent);
      }
  ...
  function injectBetterForgeButtonStyles() {
    if (!document.getElementById('betterforge-btn-css')) {
      ...
      style.textContent = `
        ...
        /* While a forge is running, the pickers are locked */
        .forge-locked .auto-upgrade-item,
        .forge-locked .auto-upgrade-tier,
        .forge-locked .auto-upgrade-stat {
          pointer-events: none;
          cursor: not-allowed;
        }
        .forge-locked .auto-upgrade-item:not(.selected),
        .forge-locked .auto-upgrade-tier:not(.selected),
        .forge-locked .auto-upgrade-stat:not(.selected) {
          opacity: 0.45;
        }
      `;
