/**
 * Cyclopedia — in-game encyclopedia mod for Bestiary Arena.
 *
 * Required globals: creatureDatabase, mapsDatabase, equipmentDatabase,
 *   inventoryDatabase, context.api.ui.components
 *
 * Lifecycle: exports.init() → open/close modal (ModalLifecycle) → exports.cleanup()
 * Preserved across modal close: monsterNameMap, MapsDataFetcher.cache, RoomThumbnailCache
 *
 * Kept fallbacks (intentional):
 ...
 *   2. Formatting & Small UI Helpers (FormatUtils, debounce, profile HTML, search bar)
 *   3. Global State & Configuration
 *   4. Lifecycle Managers
 *   5. Creature & Equipment Resolution
 *   5b. Cyclopedia API & MapsDataFetcher
 *   5c. Characters Tab Caching Utilities
 *   6. Home Search (CyclopediaHomeSearch)
 *   7. DOM/CSS Injection & Entry Points
 *   8. Layout Utilities & Modal Shell
 *   9. Shared Render Components (incl. createStartPage)
 *  10. Tab Implementations (10b–10f)
 *  11. Modal Orchestration & Teardown
...
const START_PAGE_CONFIG = {
  ...
  COLUMN_GAP: 10,
  PROFILE_COLUMN_MIN_WIDTH: '250px',
  PROFILE_COLUMN_MAX_WIDTH: '300px',
  SEARCH_COLUMN_MIN_WIDTH: '150px',
...
const LAYOUT_CONSTANTS = {
  COLUMN_WIDTH: '250px',
  LEFT_COLUMN_MIN_WIDTH: '200px',
  LEFT_COLUMN_MAX_WIDTH: '220px',
  LEFT_COLUMN_WIDTH: '200px',
  CREATURE_DETAIL_COL1_MIN_WIDTH: '250px',
  CREATURE_DETAIL_TOP_MIN_HEIGHT: 175,
  CREATURE_DETAIL_FLEX_COL_MIN: 200,
  EQUIPMENT_DETAIL_COL_MIN: 280,
  EQUIPMENT_OWNED_COL_MIN: 220,
  MAPS_DETAIL_COL_MIN: 150,
  MAPS_DETAIL_COL_MAX: 230,
  MAPS_STAT_COL_MIN: 150,
  MAPS_MAP_STATS_MIN_WIDTH: 500,
  MODAL_WIDTH: 1000,
  MODAL_HEIGHT: 660,
...
/** Shared two-column tab shell: fixed left list + flexible right detail (Bestiary pattern). */
const CYCLOPEDIA_TAB_LAYOUT = {
  container: {
    display: 'flex',
    flexDirection: 'row',
    width: '100%',
  ...
  leftCol: {
    flex: '1 1 200px',
    minWidth: LAYOUT_CONSTANTS.LEFT_COLUMN_MIN_WIDTH,
    maxWidth: LAYOUT_CONSTANTS.LEFT_COLUMN_MAX_WIDTH,
    padding: '0',
    margin: '0',
    height: '100%',
    display: 'flex',
  ...
  /** Home tab profile column: grows from 250px up to 300px before search column takes space. */
  startPageLeftCol: {
    width: START_PAGE_CONFIG.PROFILE_COLUMN_MIN_WIDTH,
    minWidth: START_PAGE_CONFIG.PROFILE_COLUMN_MIN_WIDTH,
    maxWidth: START_PAGE_CONFIG.PROFILE_COLUMN_MAX_WIDTH,
    flex: `1 0 ${START_PAGE_CONFIG.PROFILE_COLUMN_MIN_WIDTH}`,
    padding: '0',
    margin: '0',
    height: '100%',
    display: 'flex',
  ...
  /** Home tab search column: fills remaining space but never below 150px. */
  startPageRightCol: {
    flex: '1 1 0',
    minWidth: START_PAGE_CONFIG.SEARCH_COLUMN_MIN_WIDTH,
    padding: '0',
    margin: '0',
    height: '100%',
    borderImage: 'none',
...
/** Resolve equipment display name → game id (BestiaryModAPI → equipmentDatabase). */
function resolveCyclopediaEquipmentGameId(equipmentName) {
  if (!equipmentName) return null;
  const key = String(equipmentName).trim().toLowerCase();
  if (!key) return null;
  const fromApi = window.BestiaryModAPI?.utility?.maps?.equipmentNamesToGameIds?.get?.(key);
  if (fromApi != null) return fromApi;
  const fromDb = window.equipmentDatabase?.getEquipmentNameMap?.()?.get?.(key);
  if (fromDb?.gameId != null) return fromDb.gameId;
  return null;
}
...
function getCyclopediaRegionsList() {
  const stateRegions = globalThis.state?.utils?.REGIONS;
  const ordered = mapsDbRef.getRegionsInOrder?.();
  if (Array.isArray(ordered) && ordered.length > 0 && Array.isArray(stateRegions)) {
    const byId = new Map(stateRegions.map((region) => [region.id, region]));
    const result = [];
    ...
    for (const region of stateRegions) {
      if (!result.some((r) => r.id === region.id)) result.push(region);
    }
    return result;
  }
  return Array.isArray(stateRegions) ? stateRegions : [];
}

function getCyclopediaRegionIds() {
  return getCyclopediaRegionsList()
    .map((region) => region?.id)
    .filter(Boolean);
}

function findCyclopediaRegionById(regionId) {
  if (!regionId) return null;
  return getCyclopediaRegionsList().find((region) => region.id === regionId) || null;
}

function findCyclopediaRegionByMapId(mapId) {
  if (!mapId) return null;
  const regions = getCyclopediaRegionsList();
  for (const region of regions) {
    if (region.rooms?.some((room) => room.id === mapId)) {
      return region;
    }
  }
  return null;
...
function getCyclopediaMapsInRegion(regionId) {
  ...
  const roomNames = globalThis.state?.utils?.ROOM_NAME || {};
  return region.rooms.map((room) => ({
    id: room.id,
    name: roomNames[room.id] || room.id,
    region: region.id
  }));
}

function getCyclopediaRoomByMapId(mapId) {
  if (!mapId) return null;
  const fromDb = mapsDbRef.getMapById?.(mapId);
  if (fromDb) return fromDb;
  const fromRooms = globalThis.state?.utils?.ROOMS?.[mapId];
  if (fromRooms) return fromRooms;
  const region = findCyclopediaRegionByMapId(mapId);
  return region?.rooms?.find((room) => room.id === mapId) || null;
}

function getCyclopediaRoomDisplayName(roomIdOrKey) {
  const room = getCyclopediaRoomByMapId(roomIdOrKey);
  const code = room?.id ?? (typeof roomIdOrKey === 'string' && !/^\d+$/.test(roomIdOrKey) ? roomIdOrKey : null);
  const roomNames = globalThis.state?.utils?.ROOM_NAME || {};
  if (code && roomNames[code]) return roomNames[code];
  return code || String(roomIdOrKey ?? '');
}

function getCyclopediaRoomIdToRegionIdMap() {
  const map = new Map();
  getCyclopediaRegionsList().forEach((region) => {
...
function cyclopediaGetMonsterPortraitUrl(gameId, isShiny = false) {
  if (gameId == null) return '';
  if (window.creatureDatabase?.getMonsterPortraitUrl) {
    return window.creatureDatabase.getMonsterPortraitUrl(gameId, isShiny === true);
  }
  return `/assets/portraits/${gameId}${isShiny ? '-shiny' : ''}.png`;
}

/** [roomKey, room] pairs for actor scans — prefers mapsDatabase.getAllMaps(). */
function getCyclopediaAllRoomEntries() {
  const fromDb = mapsDbRef.getAllMaps?.();
  const rooms = (Array.isArray(fromDb) && fromDb.length > 0)
    ? fromDb
    : globalThis.state?.utils?.ROOMS;
  if (!rooms) return [];
  if (Array.isArray(rooms)) {
    return rooms.map((room, index) => [String(index), room]).filter(([, room]) => room);
  }
...
const cyclopediaState = {
  ...
  clearCache: function(type = 'all') {
    if (type === 'all') {
      ...
      this.cache.lastFetch.clear(); this.cache.roomThumbnails.clear();
      clearCyclopediaApiPendingRequests();
...
const CYCLOPEDIA_ASSETS = {
  ...
  starTierShiny: 'https://bestiaryarena.com/assets/icons/star-tier-shiny.png',
  starTierHundo: 'https://bestiaryarena.com/assets/icons/star-tier-hundo.png',
  starTierAwaken: 'https://bestiaryarena.com/assets/icons/star-tier-awaken.png',
  starTier: 'https://bestiaryarena.com/assets/icons/star-tier.png',
  speed: 'https://bestiaryarena.com/assets/icons/speed.png',
  grade: 'https://bestiaryarena.com/assets/icons/grade.png',
  floors: 'https://bestiaryarena.com/assets/icons/floors.png',
  raid: 'https://bestiaryarena.com/assets/icons/raid.png',
  plinko: 'https://bestiaryarena.com/assets/icons/plinko.png',
...
const MemoryUtils = {
  ...
  getMemoryInfo: function() {
    return {
      ...
      pendingRequests: cyclopediaApiPendingRequests.size
    };
  }
};
...
const DOMUtils = {
  ...
  createListItem: function(text, className = FONT_CONSTANTS.SIZES.BODY, isOwned = true, isPerfect = false, isT5 = false, hasShiny = false, hasAwakened = false, hasShinyTier = false, hasHundoTier = false) {
    ...
    if (hasShiny) {
      ...
      shinyIcon.src = CYCLOPEDIA_ASSETS.shinyStar;
      shinyIcon.alt = 'shiny';
...
function openCreatureInBestiaryTab(creatureNameRaw, {
  ...
  const resolveTabPage = () => (
    typeof getTabPageFn === 'function' ? getTabPageFn() : tabPage
  );
  const resolveInventoryTabPage = () => (
    typeof getInventoryTabPageFn === 'function' ? getInventoryTabPageFn() : inventoryTabPage
  );
  ...
  if (isCyclopediaGazerCreatureName(canonicalCreatureName)) {
    ...
    const clickGazerTimeout = setTimeout(() => {
      const inventoryTab = resolveInventoryTabPage() || resolveTabPage();
  ...
  const clickBestiaryTimeout = setTimeout(() => {
    const page = resolveTabPage();
    if (typeof clickListItemFn === 'function' && page) {
      clickListItemFn(page, canonicalCreatureName);
    }
  }, 0);
...
// =======================
// --- Event Handler Management (modal-scoped) ---
// =======================
...
const CYCLOPEDIA_GLOBAL_TIMER_KEYS = new Set(['cacheCleanup', 'domCacheCleanup']);
const CYCLOPEDIA_REFRESH_ON_OPEN_MS = 60000;

let activeModalLifecycle = null;
...
const ModalLifecycle = {
  create() {
    return {
      ...
      addHandlerKey(key) {
        if (key) this.handlerKeys.push(key);
      },
      ...
      registerTabPage(pageEl) {
        if (pageEl) this.tabPages.push(pageEl);
      },
      ...
      teardown() {
        ...
        for (const unsub of this.subscriptions) {
          try { unsub(); } catch (error) { /* ignore */ }
        }
        this.subscriptions.length = 0;

        for (const page of this.tabPages) {
          try {
            if (typeof page.cleanupCharactersTab === 'function') page.cleanupCharactersTab();
            if (typeof page._cleanupSubscription === 'function') page._cleanupSubscription();
            page._cyclopediaStartPageManager?.cleanup?.();
            if (typeof page._cleanup === 'function') page._cleanup();
          } catch (error) {
            console.warn('[Cyclopedia] Error during tab page cleanup:', error);
          }
        }
        this.tabPages.length = 0;

        for (let i = this.cleanups.length - 1; i >= 0; i--) {
          try { this.cleanups[i](); } catch (error) { /* ignore */ }
        }
        this.cleanups.length = 0;

        for (const key of this.handlerKeys) {
          EventHandlerManager.removeHandler(key);
        }
        this.handlerKeys.length = 0;
...
function registerModalObserver(observer, name = 'modalObserver') {
  if (activeModalLifecycle?.isActive) {
    activeModalLifecycle.addObserver(observer);
    return;
  }
  ObserverManager.add(observer, name);
}

function registerModalAbortController(controller) {
  if (activeModalLifecycle?.isActive && controller) {
    activeModalLifecycle.addAbortController(controller);
  }
  return controller;
}

function getModalFetchSignal() {
  return registerModalAbortController(new AbortController()).signal;
}

function registerModalHandler(element, eventType, handler, options = {}) {
  const key = EventHandlerManager.addHandler(element, eventType, handler, options);
  if (activeModalLifecycle?.isActive) {
    activeModalLifecycle.addHandlerKey(key);
  }
  return key;
}

function clearCyclopediaModalTimers() {
  for (const [key, timerId] of [...cyclopediaState.timers.entries()]) {
    if (CYCLOPEDIA_GLOBAL_TIMER_KEYS.has(key)) continue;
    try { clearInterval(timerId); } catch (error) { /* ignore */ }
    cyclopediaState.timers.delete(key);
  }
}

function isCyclopediaRateLimited() {
  return Date.now() < (cyclopediaState.rateLimitedUntil || 0);
}

function applyCyclopediaRateLimitFromResponse(response) {
  if (!response || response.status !== 429) return;
  const retryAfter = Number(response.headers?.get?.('retry-after')) || 30;
  cyclopediaState.rateLimitedUntil = Date.now() + retryAfter * 1000;
}
...
async function getHuntingMarksFromProfileAPI() {
  try {
    ...
    const data = await CyclopediaApi.fetchProfile(playerName);
...
const cyclopediaApiPendingRequests = new Map();

function clearCyclopediaApiPendingRequests() {
...
async function fetchWithDeduplication(url, key, priority = 0, signal = null) {
  try {
    ...
    if (cyclopediaApiPendingRequests.has(key)) return await cyclopediaApiPendingRequests.get(key);
    ...
    const requestPromise = fetch(url, fetchOptions).then(async (res) => {
      if (res.status === 429) {
        applyCyclopediaRateLimitFromResponse(res);
      }
      if (!res.ok) {
      ...
      throw error;
    }).finally(() => cyclopediaApiPendingRequests.delete(key));
...
const CyclopediaApi = {
  buildProfileUrl(playerName) {
    return `${START_PAGE_CONFIG.API_BASE_URL}?batch=1&input=%7B%220%22%3A%7B%22json%22%3A%22${encodeURIComponent(playerName)}%22%7D%7D`;
  },
  ...
  parseProfileResponse(data) {
    ...
    return unwrapProfilePageJson(data);
  },

  async fetchProfile(playerName, { priority = 1, signal = null } = {}) {
    if (!playerName || typeof playerName !== 'string') {
      throw new Error('Invalid player name provided');
    }

    const cached = cyclopediaState.getProfileData(playerName);
    if (cached) return this.parseProfileResponse(cached);

    const effectiveSignal = signal || (activeModalLifecycle?.isActive ? getModalFetchSignal() : null);
    const raw = await fetchWithDeduplication(
      this.buildProfileUrl(playerName),
      `profile-${playerName}`,
      priority,
      effectiveSignal
    );

    const profile = this.parseProfileResponse(raw);
    if (profile?.name) {
      cyclopediaState.setProfileData(playerName, profile);
    }
    return profile;
  },

  fetchTRPC(method) {
    return MapsDataFetcher.fetchTRPC(method);
  },

  fetchLeaderboardBundle() {
    return MapsDataFetcher.fetchAllLeaderboardData();
  },

  async fetchRankings(options = {}) {
...
const CreatureListManager = {
  ...
  createSimplifiedCreatureIcon(actor) {
    ...
    img.src = cyclopediaGetMonsterPortraitUrl(actor.id, actor.shiny === true);
    img.alt = 'creature';
    img.style.cssText = `
      width: 100%;
      height: 100%;
...
const buildCyclopediaMonsterNameMap = MemoizationUtils.memoize(function() {
  ...
  if (typeof db?.buildMonsterNameMap === 'function') {
    cyclopediaState.monsterNameMap = db.buildMonsterNameMap();
    cyclopediaState.monsterNameMapBuilt = true;
    return cyclopediaState.monsterNameMap;
  }
    ...
    getCyclopediaAllRoomEntries().forEach(([roomKey, room]) => {
      try {
        ...
        if (monsterInRoom.length > 0) {
          ...
          locations.push({ roomId: roomKey, roomName, positions: roomLocations });
        }
      } catch (error) {
...
function refreshCyclopediaRecordsOnOpen() {
  const now = Date.now();
  if (cyclopediaState.lastModalOpenAt && now - cyclopediaState.lastModalOpenAt < CYCLOPEDIA_REFRESH_ON_OPEN_MS) {
    return;
  }
  cyclopediaState.lastModalOpenAt = now;
  // Refresh WR + personal records when Cyclopedia opens (keep rankings cache).
...
const MapsDataFetcher = {
  ...
  async fetchTRPC(method) {
    if (isCyclopediaRateLimited()) {
      const stale = this.getCached(method);
  ...
  async _makeTRPCRequest(method) {
    ...
    try {
      ...
      if (response.status === 429) {
        applyCyclopediaRateLimitFromResponse(response);
...
const CyclopediaHomeSearch = (() => {
  ...
  function buildRoomIdToRegionId() {
    return getCyclopediaRoomIdToRegionIdMap();
  }
  ...
  function resolveEquipmentIdByName(equipmentName) {
    try {
      return resolveCyclopediaEquipmentGameId(equipmentName);
    } catch {
      return null;
    }
  }
  ...
  function buildBaseEntries() {
    ...
    const roomIdToRegionId = buildRoomIdToRegionId();
    ...
    regions.forEach((region) => {
      ...
      rooms.forEach((room) => {
        ...
        try {
          ...
          const diff = getCyclopediaRoomByMapId(roomId)?.difficulty;
...
const CYCLOPEDIA_MAPS_ROW_CSS = `.cyclopedia-maps-list { container-type: inline-size; container-name: cyclopedia-maps-list; width: 100%; } .cyclopedia-map-row { display: grid; grid-template-columns: 64px minmax(0, 1fr) auto auto auto; align-items: center; gap: 8px; padding: 8px; border: 2px solid #666; border-radius: 4px; cursor: pointer; transition: background-color 0.2s; box-sizing: border-box; width: 100%; min-width: 0; } .cyclopedia-map-thumb { width: 64px; height: 64px; object-fit: cover; border: 1px solid #666; border-radius: 4px; } .cyclopedia-map-name { font-size: 14px; font-weight: bold; hyphens: auto; word-break: break-word; overflow-wrap: anywhere; min-width: 0; } .cyclopedia-map-stats { display: contents; } .cyclopedia-map-stat { display: flex; flex-direction: column; align-items: center; justify-content: center; padding: 4px; min-width: 40px; max-width: 100px; font-size: 9px; font-family: 'Trebuchet MS', 'Arial Black', Arial, sans-serif; text-align: center; box-sizing: border-box; } .cyclopedia-map-stat--floors { min-width: 40px; } @container cyclopedia-maps-list (max-width: 400px) { .cyclopedia-map-row { grid-template-columns: 64px minmax(0, 1fr); grid-template-rows: auto auto; align-items: start; } .cyclopedia-map-stats { display: flex; flex-direction: row; flex-wrap: wrap; justify-content: space-evenly; align-items: flex-start; gap: 8px; grid-column: 1 / -1; width: 100%; padding-top: 8px; margin-top: 4px; border-top: 1px solid rgba(102, 102, 102, 0.45); } .cyclopedia-map-stat { flex: 1 1 40px; max-width: none; min-width: 40px; } }`;
function injectCyclopediaMapsRowStyles() { injectCyclopediaStyle('cyclopedia-maps-row-css', CYCLOPEDIA_MAPS_ROW_CSS); }
  ...
  const ownedRooms = (type === 'map' || type === 'region')
    ...
    items.forEach((name, itemIndex) => {
      ...
      if (type === 'creature') {
        ...
        const region = regionId ? findCyclopediaRegionById(regionId) : null;
        if (region?.rooms) {
        ...
        if (mapId && isMapRaid(mapId)) {
          ...
          if (contentContainer && contentContainer.tagName === 'DIV') {
            ...
            if (isDynamicEvent) {
              ...
              icon.src = CYCLOPEDIA_ASSETS.plinko;
              icon.alt = 'event';
              icon.title = 'Event map';
            } else {
              // Use raid icon for static raids
              icon.src = CYCLOPEDIA_ASSETS.raid;
              icon.alt = 'raid';
              icon.title = 'Raid map';
            }
...
const CYCLOPEDIA_STACKABLE_COL_PROPS = [
  ...
  'overflow', 'overflowX', 'overflowY', 'borderRight', 'borderBottom', 'borderImage'
];

const CYCLOPEDIA_RELAX_SCROLL_PROPS = ['overflowY', 'overflowX', 'flex', 'height', 'maxHeight', 'minHeight'];

function snapshotElementLayoutStyles(el) {
  const snapshot = {};
  CYCLOPEDIA_STACKABLE_COL_PROPS.forEach((prop) => {
    snapshot[prop] = el.style[prop] || '';
  });
  return snapshot;
}

function applyElementLayoutStyles(el, snapshot) {
  Object.assign(el.style, snapshot);
}

function setCyclopediaInnerScrollRelaxed(els, relaxed) {
  const elements = (typeof els === 'function' ? els() : els).filter(Boolean);
  elements.forEach((el) => {
    if (relaxed) {
      if (el.dataset.cyclopediaRelaxed !== '1') {
        CYCLOPEDIA_RELAX_SCROLL_PROPS.forEach((prop) => {
          el.dataset[`cyclopediaRow${prop}`] = el.style[prop] || '';
        });
        el.dataset.cyclopediaRelaxed = '1';
      }
      ...
      el.style.minHeight = el.dataset.cyclopediaRelaxedMinHeight || '0';
    } else if (el.dataset.cyclopediaRelaxed === '1') {
      CYCLOPEDIA_RELAX_SCROLL_PROPS.forEach((prop) => {
        const saved = el.dataset[`cyclopediaRow${prop}`];
        if (saved != null) el.style[prop] = saved;
        delete el.dataset[`cyclopediaRow${prop}`];
      });
      delete el.dataset.cyclopediaRelaxed;
    }
  });
}

function shouldRelaxStackedFlexChild(el) {
  const flex = el.style.flex || '';
  if (!flex) return false;
  if (flex.includes('%')) return true;
  return /(^|\s)1\s+1\s+/.test(flex) || flex === '1' || flex.startsWith('1 ');
}

function relaxStackedColumnChildren(parent, relaxed) {
  if (!parent) return;
  Array.from(parent.children).forEach((child) => {
    if (shouldRelaxStackedFlexChild(child)) {
      setCyclopediaInnerScrollRelaxed([child], relaxed);
    }
  });
}

function setupCyclopediaStackableColumnsLayout({ container, columns, stackThreshold, relaxScroll = [], afterApply }) {
  if (!container || !columns?.length) return;

  const frameBorderRight = '6px solid transparent';
  const frameBorderImage = `url("${START_PAGE_CONFIG.FRAME_IMAGE_URL}") 6 6 6 6 fill stretch`;
  const rowSnapshots = columns.map((col) => snapshotElementLayoutStyles(col.el));
  let stacked = null;
  ...
  const apply = () => {
    ...
    const shouldStack = width < stackThreshold;
    const modeChanged = shouldStack !== stacked;
    ...
    if (shouldStack) {
      ...
      columns.forEach((col, index) => {
        const isLast = col.noRightBorder || index === columns.length - 1;
        Object.assign(col.el.style, stackedColStyle);
        col.el.style.borderRight = 'none';
        col.el.style.borderBottom = isLast ? 'none' : frameBorderRight;
        col.el.style.borderImage = isLast ? 'none' : frameBorderImage;
      });

      columns.forEach((col) => relaxStackedColumnChildren(col.el, true));
      setCyclopediaInnerScrollRelaxed(relaxScroll, true);
      stacked = true;
    } else if (modeChanged) {
      ...
      container.style.overflowX = 'hidden';
      columns.forEach((col, index) => {
        applyElementLayoutStyles(col.el, rowSnapshots[index]);
      });
      columns.forEach((col) => relaxStackedColumnChildren(col.el, false));
      setCyclopediaInnerScrollRelaxed(relaxScroll, false);
      stacked = false;
    }
  ...
  if (typeof ResizeObserver !== 'undefined') {
    ...
    registerModalObserver(observer, 'cyclopediaStackableColumns');
  } else {
    registerModalHandler(window, 'resize', apply);
  }
...
/**
 * Standard Cyclopedia tab layout: left list column + right detail column(s).
 * @param {object} options
 * @param {boolean} [options.rightColRow=false] - horizontal flex row on rightCol
 * @param {(leftCol: HTMLElement) => void} [options.mountLeft]
 * @param {(rightCol: HTMLElement) => void} [options.mountRight]
 * @param {object} [options.stackConfig] - passed to setupCyclopediaStackableColumnsLayout on rightCol
 */
function createCyclopediaTabShell({ rightColRow = false, mountLeft, mountRight, stackConfig } = {}) {
  const root = document.createElement('div');
  Object.assign(root.style, CYCLOPEDIA_TAB_LAYOUT.container);

  const leftCol = document.createElement('div');
  Object.assign(leftCol.style, CYCLOPEDIA_TAB_LAYOUT.leftCol);

  const rightCol = document.createElement('div');
  Object.assign(
    rightCol.style,
    CYCLOPEDIA_TAB_LAYOUT.rightCol,
    rightColRow ? CYCLOPEDIA_TAB_LAYOUT.rightColRow : {}
  );

  if (typeof mountLeft === 'function') mountLeft(leftCol);
  if (typeof mountRight === 'function') mountRight(rightCol);

  root.appendChild(leftCol);
  root.appendChild(rightCol);

  if (stackConfig) {
    rightCol._relayoutStackableColumns = setupCyclopediaStackableColumnsLayout({
      container: rightCol,
      ...stackConfig
    });
  }
...
function cyclopediaFindBoxByTitle(tabRoot, titleText) {
...
function cyclopediaFindClickableListItems(root) {
  if (!root) return [];
  return Array.from(root.querySelectorAll('div.pixel-font-16'));
}

function cyclopediaClickListItem(root, desiredText) {
  const want = cyclopediaNormalizeSearchText(desiredText);
  if (!want) return false;

  const items = cyclopediaFindClickableListItems(root);
...
function cyclopediaNavigateFromHome(target, { setActiveTab, tabPages }) {
  if (!target || typeof target !== 'object') return false;

  try {
    if (target.type === 'creature') {
      setActiveTab(1);
      return cyclopediaClickListItem(tabPages[1], target.name);
    }

    if (target.type === 'equipment') {
      setActiveTab(2);
      if (typeof window !== 'undefined') window.cyclopediaSelectedEquipment = target.name;
      return cyclopediaClickListItem(tabPages[2], target.name);
    }

    if (target.type === 'inventoryCategory') {
      setActiveTab(3);
      const inventoryTab = tabPages[3];
      const topBox = cyclopediaFindBoxByTitle(inventoryTab, 'Inventory');
      return cyclopediaClickListItem(topBox || inventoryTab, target.categoryName);
    }
    ...
    if (target.type === 'inventoryItem') {
      ...
      const ok = cyclopediaClickListItem(topBox || inventoryTab, target.categoryName);
      const clickItemTimeout = setTimeout(() => {
        const bottomBox = cyclopediaFindBoxByTitle(inventoryTab, target.categoryName) || inventoryTab;
        cyclopediaClickListItem(bottomBox, catalogLabel);
    ...
    if (target.type === 'region') {
      ...
      const regionsBox = cyclopediaFindBoxByTitle(mapsTab, 'Regions');
      return cyclopediaClickListItem(regionsBox || mapsTab, target.regionName);
    }
    ...
    if (target.type === 'map') {
      ...
      const mapsTab = tabPages[4];
      const regionsBox = cyclopediaFindBoxByTitle(mapsTab, 'Regions');
      const ok = cyclopediaClickListItem(regionsBox || mapsTab, target.regionName);
...
function createStartPageManager() {
  ...
  class StartPageManager {
    ...
    createContainer() {
      ...
      Object.assign(container.style, CYCLOPEDIA_TAB_LAYOUT.container);
      return container;
    }
    ...
    async fetchProfileData(playerName) {
        ...
        registerModalAbortController(controller);

        try {
          const profile = await CyclopediaApi.fetchProfile(playerName, {
            priority: 1,
    ...
    async initialize() {
      try {
        ...
        const profileData = CyclopediaApi.parseProfileResponse(data);
    ...
    renderLayout(playerName, profileData) {
        ...
        const leftCol = document.createElement('div');
        Object.assign(leftCol.style, CYCLOPEDIA_TAB_LAYOUT.startPageLeftCol);
        leftCol.appendChild(renderCyclopediaProfileColumn(profileData));

        const rightCol = document.createElement('div');
        Object.assign(rightCol.style, CYCLOPEDIA_TAB_LAYOUT.startPageRightCol);
        rightCol.appendChild(renderCyclopediaSearchColumn());
    ...
    startTimer() {
        ...
        this._countdownTimerId = timerId;
        TimerManager.addInterval(timerId, 'startPageCountdown');
    }

    stopTimer() {
        if (this._countdownTimerId != null) {
          TimerManager.remove(this._countdownTimerId);
...
function createStartPage() {
  ...
  startPage.container._cyclopediaStartPageManager = startPage;
...
function renderCreatureTemplate(name, showShinyPortraits = false) {
  ...
  Object.assign(col1.style, creatureTemplateColStyle, {
    flex: `0 0 ${LAYOUT_CONSTANTS.CREATURE_DETAIL_COL1_MIN_WIDTH}`,
    width: LAYOUT_CONSTANTS.COLUMN_WIDTH,
    minWidth: LAYOUT_CONSTANTS.CREATURE_DETAIL_COL1_MIN_WIDTH,
    maxWidth: 'none'
  });
  ...
  col1TopArea.style.minHeight = `${LAYOUT_CONSTANTS.CREATURE_DETAIL_TOP_MIN_HEIGHT}px`;
  col1TopArea.style.height = 'auto';
  col1TopArea.style.flexShrink = '0';
  ...
  abilitySection.style.flex = '0 0 auto';
  ...
  awakenIconImg.src = CYCLOPEDIA_ASSETS.starTierAwaken;
  ...
  const abilityScrollMaxHeight = 250;
  fitAbilityScrollContainer = () => {
    abilityScrollContainer.style.height = 'auto';
    const needed = abilityScrollContainer.scrollHeight;
    if (needed <= abilityScrollMaxHeight) {
      abilityScrollContainer.style.height = `${needed}px`;
      abilityScrollContainer.style.overflowY = 'hidden';
    } else {
      abilityScrollContainer.style.height = `${abilityScrollMaxHeight}px`;
      abilityScrollContainer.style.overflowY = 'auto';
    }
  };
  abilityScrollContainer.style.maxHeight = `${abilityScrollMaxHeight}px`;
  abilityScrollContainer.style.height = 'auto';
  abilityScrollContainer.style.overflowY = 'auto';
    ...
    if (regionMap.size > 0) {
      for (const [regionName, rooms] of regionMap.entries()) {
        ...
        rooms.forEach(({ roomName, positions }) => {
          ...
          if (foundRoom?.staminaCost != null) {
            staminaCost = foundRoom.staminaCost;
          }
          if (staminaCost != null) {
            const icon = document.createElement('img');
            icon.src = CYCLOPEDIA_ASSETS.stamina;
            icon.alt = 'Stamina';
          ...
          [...levelCounts.entries()]
            ...
            detail.appendChild(levelSpan);
            if (count > 1) {
              const multSpan = document.createElement('span');
              multSpan.textContent = `×${count}`;
              multSpan.style.color = COLOR_CONSTANTS.PRIMARY;
              multSpan.style.fontWeight = 'bold';
              multSpan.style.textShadow = '1px 1px 0 #000, -1px -1px 0 #000, 1px -1px 0 #000, -1px 1px 0 #000, 0 0 4px rgba(0, 0, 0, 0.6)';
              detail.appendChild(multSpan);
            }
  ...
  function getBoardConfigUsageStatsForCreature(targetMonsterGameId) {
    ...
    try {
      ...
      Object.entries(boardConfigs).forEach(([roomCode, cfgs]) => {
        ...
        if (isDynamicEventMap(roomCode)) return;
        total++;
  ...
  if (ownedMonsters.length > 0) {
    ...
    ownedMonsters.forEach(monster => {
      ...
      if (isMaxAwakened) {
        ...
        if (starTierIcon) {
          if (monster.shiny === true) {
            starTierIcon.src = CYCLOPEDIA_ASSETS.starTierShiny;
            starTierIcon.alt = 'shiny-tier';
            starTierIcon.title = 'Has level 99 max-genes shiny creature';
          } else {
            starTierIcon.src = CYCLOPEDIA_ASSETS.starTierHundo;
  ...
  const creatureCol1Width = parseInt(LAYOUT_CONSTANTS.COLUMN_WIDTH, 10) || 250;
  setupCyclopediaStackableColumnsLayout({
    container,
    stackThreshold: creatureCol1Width + (LAYOUT_CONSTANTS.CREATURE_DETAIL_FLEX_COL_MIN * 2),
    columns: [
      { el: col1 },
      { el: col2 },
      { el: col3, noRightBorder: true }
    ],
    relaxScroll: [dropsSection, dropsList, col3Content]
  });
...
function renderCyclopediaProfileColumn(profileData) {
  ...
  Object.assign(linkFooter.style, {
    display: 'flex',
    alignItems: 'center',
    justifyContent: 'center',
    flexShrink: '0',
  ...
  linkFooter.appendChild(createCyclopediaSeasonsLeaderboardLink());
  wrapper.appendChild(linkFooter);
  return wrapper;
}
...
function resolveCyclopediaEquipmentSpriteId(equipmentName) {
  if (!equipmentName) return null;
  const gameId = resolveCyclopediaEquipmentGameId(equipmentName);
...
function cyclopediaCreateCreaturePortrait(creatureName, sizePx = CYCLOPEDIA_HOME_SEARCH_ICON_PX) {
  ...
  try {
    ...
    const img = document.createElement('img');
    img.src = cyclopediaGetMonsterPortraitUrl(gameId, useShinyPortrait);
...
function renderDailyContextColumn() {
  ...
  const formatTime = FormatUtils.time;
...
function createStatisticsSection(selectedMap, leaderboardData) {
  ...
  const mapStatsMinWidth = `${LAYOUT_CONSTANTS.MAPS_MAP_STATS_MIN_WIDTH}px`;
  const statsContainer = document.createElement('div');
  statsContainer.className = 'cyclopedia-map-stats-panel';
  statsContainer.style.display = 'flex';
  statsContainer.style.flexDirection = 'column';
  statsContainer.style.width = '100%';
  statsContainer.style.minWidth = '0';
  statsContainer.style.height = '100%';
  statsContainer.style.boxSizing = 'border-box';
  const MAPS_TABLE_HEADER_GRID_BORDER = '1px solid #444';
  const MAPS_TABLE_ROW_GRID_BORDER = '1px solid #333';
  ...
  row1.style.minWidth = '0';
  row1.style.maxWidth = '100%';
  row1.style.width = '100%';
  row1.style.boxSizing = 'border-box';
  row1.style.overflowX = 'visible';
  ...
  speedrunCol.style.flex = '0 0 160px';
  speedrunCol.style.width = '160px';
  speedrunCol.style.maxWidth = '160px';
  speedrunCol.style.minWidth = '160px';
  speedrunCol.style.boxSizing = 'border-box';
  ...
  speedIcon.src = CYCLOPEDIA_ASSETS.speed;
  speedIcon.alt = 'Speed';
  ...
  rankPointsCol.style.flex = '0 0 160px';
  rankPointsCol.style.width = '160px';
  rankPointsCol.style.maxWidth = '160px';
  rankPointsCol.style.minWidth = '160px';
  rankPointsCol.style.boxSizing = 'border-box';
  ...
  gradeIcon.src = CYCLOPEDIA_ASSETS.grade;
  gradeIcon.alt = 'Grade';
  ...
  floorsCol.style.flex = '0 0 180px';
  floorsCol.style.width = '180px';
  floorsCol.style.maxWidth = '180px';
  floorsCol.style.minWidth = '180px';
  floorsCol.style.boxSizing = 'border-box';
  ...
  floorsIcon.src = CYCLOPEDIA_ASSETS.floors;
  ...
  row2.style.minWidth = mapStatsMinWidth;
  row2.style.maxWidth = '100%';
  row2.style.width = '100%';
  row2.style.boxSizing = 'border-box';
  row2.style.overflowX = 'visible';
  ...
  speedrunTableCol.style.flex = '0 0 160px';
  speedrunTableCol.style.width = '160px';
  speedrunTableCol.style.maxWidth = '160px';
  speedrunTableCol.style.minWidth = '160px';
  speedrunTableCol.style.boxSizing = 'border-box';
  ...
  const speedrunHeader = DOMUtils.createStyledElement('div', {
    ...
    borderBottom: MAPS_TABLE_HEADER_GRID_BORDER,
    position: 'sticky',
    top: '0',
    zIndex: '2',
    fontWeight: 'bold',
  ...
  const timeHeader = DOMUtils.createTableCell('Time', { padding: '6px 4px', borderRight: MAPS_TABLE_HEADER_GRID_BORDER });
  speedrunHeader.appendChild(timeHeader);
  
  const copyHeader = DOMUtils.createTableCell('', { padding: '4px 2px', borderRight: MAPS_TABLE_HEADER_GRID_BORDER });
    ...
    try {
      ...
      if (!localRuns || localRuns.length === 0) {
        ...
        for (let i = 1; i <= 5; i++) {
          ...
          row.style.minHeight = '20px';
          row.style.boxSizing = 'border-box';
          row.style.borderBottom = i < 5 ? MAPS_TABLE_ROW_GRID_BORDER : 'none';
          ...
          timeCell.style.borderRight = MAPS_TABLE_ROW_GRID_BORDER;
          ...
          copyCell.style.borderRight = MAPS_TABLE_ROW_GRID_BORDER;
      ...
      for (let i = 0; i < 5; i++) {
        ...
        row.style.minHeight = '20px';
        row.style.boxSizing = 'border-box';
        row.style.borderBottom = i < 4 ? MAPS_TABLE_ROW_GRID_BORDER : 'none';
        row.style.fontSize = '10px';
        row.style.fontFamily = "'Trebuchet MS', 'Arial Black', Arial, sans-serif";
        ...
        if (run.time) {
          ...
          timeText.textContent = FormatUtils.ticks(run.time);
          ...
          copyCell.addEventListener('click', (e) => {
            ...
            if (run.regionName) {
              ...
              try {
                ...
                const foundRegion = findCyclopediaRegionByMapId(mapId);
  ...
  floorsTableCol.style.flex = '0 0 180px';
  floorsTableCol.style.width = '180px';
  floorsTableCol.style.maxWidth = '180px';
  floorsTableCol.style.minWidth = '180px';
  floorsTableCol.style.boxSizing = 'border-box';
  floorsTableCol.style.overflow = 'hidden';
  ...
  const floorsFloorHeader = DOMUtils.createTableCell('Floor', { padding: '6px 4px', borderRight: MAPS_TABLE_HEADER_GRID_BORDER });
  floorsHeader.appendChild(floorsFloorHeader);
  
  const floorsTimeHeader = DOMUtils.createTableCell('Time', { padding: '6px 4px', borderRight: MAPS_TABLE_HEADER_GRID_BORDER });
  floorsHeader.appendChild(floorsTimeHeader);
  
  const floorsCopyHeader = DOMUtils.createTableCell('', { padding: '4px 2px', borderRight: MAPS_TABLE_HEADER_GRID_BORDER });
    ...
    try {
      ...
      if (!localRuns || localRuns.length === 0) {
        ...
        for (let i = 1; i <= 5; i++) {
          ...
          floorCell.style.borderRight = MAPS_TABLE_ROW_GRID_BORDER;
          ...
          ticksCell.style.borderRight = MAPS_TABLE_ROW_GRID_BORDER;
      ...
      for (let i = 0; i < rowCount; i++) {
        ...
        row.style.minHeight = '20px';
        row.style.boxSizing = 'border-box';
        row.style.borderBottom = i < rowCount - 1 ? MAPS_TABLE_ROW_GRID_BORDER : 'none';
        row.style.fontSize = '10px';
        row.style.fontFamily = "'Trebuchet MS', 'Arial Black', Arial, sans-serif";
  ...
  ranksTableCol.style.flex = '0 0 160px';
  ranksTableCol.style.width = '160px';
  ranksTableCol.style.maxWidth = '160px';
  ranksTableCol.style.minWidth = '160px';
  ranksTableCol.style.boxSizing = 'border-box';
  ...
  ranksTable.style.overflowY = 'visible';
  ranksTable.style.boxSizing = 'border-box';
  ranksTable.style.padding = '0 1px';
  ranksTable.style.height = '140px';
  ranksTable.style.maxHeight = '140px';
  ...
  ranksHeader.style.height = '25px';
  ranksHeader.style.minHeight = '25px';
  ranksHeader.style.boxSizing = 'border-box';
  ranksHeader.style.background = 'url("https://bestiaryarena.com/_next/static/media/background-regular.b0337118.png") repeat';
  ranksHeader.style.borderBottom = MAPS_TABLE_HEADER_GRID_BORDER;
  ...
  ranksRankHeader.style.borderRight = MAPS_TABLE_HEADER_GRID_BORDER;
  ...
  ranksTimeHeader.style.borderRight = MAPS_TABLE_HEADER_GRID_BORDER;
  ...
  ranksCopyHeader.style.borderRight = MAPS_TABLE_HEADER_GRID_BORDER;
    ...
    try {
      ...
      if (!localRuns || localRuns.length === 0) {
        ...
        for (let i = 1; i <= 5; i++) {
          ...
          rankCell.style.borderRight = MAPS_TABLE_ROW_GRID_BORDER;
  ...
  const applyMapsLeaderboardData = (data) => {
    if (data && selectedMap) {
  ...
  const onLeaderboardError = (error) => {
  ...
  const populateAllLocalRunTables = () => {
    populateSpeedrunTable();
    populateRankPointsTable();
    populateFloorsTable();
  };

  if (arguments.length >= 2 && leaderboardData != null) {
    applyMapsLeaderboardData(leaderboardData);
  } else if (MapsTabDOMOptimizer.leaderboardBundle) {
    applyMapsLeaderboardData(MapsTabDOMOptimizer.leaderboardBundle);
  } else {
    fetchMapsLeaderboardData()
      .then((data) => {
        MapsTabDOMOptimizer.leaderboardBundle = data;
        applyMapsLeaderboardData(data);
      })
      .catch(onLeaderboardError);
    populateAllLocalRunTables();
  }
      ...
      try {
        ...
        nameElement.addEventListener('click', () => {
          ...
          openCreatureInBestiaryTab(creatureName, {
            ...
            clickListItemFn: cyclopediaClickListItem,
            getTabPageFn: () => tabPages[1],
            getInventoryTabPageFn: () => tabPages[3],
            findBoxByTitleFn: cyclopediaFindBoxByTitle,
          ...
          if (equipmentName) {
            ...
            const handleEquipmentClick = () => {
              ...
              const clickEquipmentTimeout = setTimeout(() => {
                if (typeof tabPages !== 'undefined' && tabPages[2]) {
                  cyclopediaClickListItem(tabPages[2], equipmentName);
                }
              }, 0);
...
function createCompactStatColumn(iconSrc, iconAlt, mapId, mapData, leaderboardData) {
  ...
  statCol.className = 'cyclopedia-map-stat' + (iconAlt === 'Floors' ? ' cyclopedia-map-stat--floors' : '');
...
function createRegionMapsSection(mapsInRegion, regionId, onMapSelect, leaderboardData) {
  ...
  mapsContainer.className = 'cyclopedia-maps-list';
  ...
  const renderMapItems = (data) => {
    mapsToDisplay.forEach(map => {
      const mapItem = document.createElement('div');
      mapItem.className = 'cyclopedia-map-row';
      ...
      thumbnail.className = 'pixelated cyclopedia-map-thumb';
      mapItem.appendChild(thumbnail);
      
      // Map name (Column 2)
      const mapName = document.createElement('div');
      mapName.className = 'cyclopedia-map-name';
      ...
      statsRow.className = 'cyclopedia-map-stats';
      
      // Speedrun column (Column 3)
      statsRow.appendChild(createCompactStatColumn(
        CYCLOPEDIA_ASSETS.speed,
        'Speed',
        map.id,
        mapData,
        data
  ...
  if (arguments.length >= 4) {
    renderMapItems(leaderboardData);
  } else {
    fetchMapsLeaderboardData().then(renderMapItems).catch(error => {
...
function createRegionStatisticsSection(regionId, mapsInRegion, leaderboardData) {
  const statsContainer = document.createElement('div');
  statsContainer.style.display = 'flex';
  statsContainer.style.flexDirection = 'column';
  statsContainer.style.width = '100%';
  statsContainer.style.flex = '0 0 auto';
  ...
  nonRaidMaps.forEach(map => {
    ...
    const roomData = getCyclopediaRoomByMapId(map.id);
  ...
  const applyLeaderboardWRTicks = (data) => {
    nonRaidMaps.forEach(map => {
      const bestTicks = data?.best?.[map.id]?.ticks || 0;
      if (bestTicks > 0) {
...
function createMapInfoSection(selectedMap, roomName) {
  ...
  mapInfoDiv.style.flexDirection = 'column';
  mapInfoDiv.style.alignItems = 'center';
  ...
  thumbnailContainer.style.minWidth = '192px';
  thumbnailContainer.style.minHeight = '192px';
  thumbnailContainer.style.maxWidth = '192px';
  thumbnailContainer.style.maxHeight = '192px';
  thumbnailContainer.style.flex = '0 0 192px';
  ...
  thumbnailRow.style.width = '100%';
  thumbnailRow.style.display = 'flex';
  thumbnailRow.style.justifyContent = 'center';
  thumbnailRow.style.alignItems = 'center';
  thumbnailRow.appendChild(thumbnailContainer);
  mapInfoDiv.appendChild(thumbnailRow);
...
function createMapsTabPage(selectedCreature, selectedEquipment, selectedInventory, setSelectedCreature, setSelectedEquipment, setSelectedInventory, updateRightCol, mapsTabNav = {}) {
  ...
  MapsTabDOMOptimizer.leaderboardBundle = null;

  let selectedCategory = '';
  let selectedMap = null;

  const { root: d, leftCol, rightCol } = createCyclopediaTabShell({ rightColRow: true });
  ...
  function updateBottomBox() {
    try {
      ...
      if (selectedCategory) {
        mapsInRegion = getCyclopediaMapsInRegion(selectedCategory);
      }
  ...
  let regions = getCyclopediaRegionIds();
  ...
  const mapsNarrowColStyle = {
    flex: `1 1 ${LAYOUT_CONSTANTS.MAPS_DETAIL_COL_MIN}px`,
    minWidth: `${LAYOUT_CONSTANTS.MAPS_DETAIL_COL_MIN}px`,
    maxWidth: `${LAYOUT_CONSTANTS.MAPS_DETAIL_COL_MAX}px`,
    width: 'auto'
  };
  const mapsWideColStyle = {
    flex: '2 1 0',
    minWidth: '0',
    maxWidth: 'none',
    width: 'auto'
  ...
  const col2 = document.createElement('div');
  Object.assign(col2.style, mapsNarrowColStyle, {
  ...
  const col3 = document.createElement('div');
  Object.assign(col3.style, mapsWideColStyle, {
    height: '100%', display: 'flex', flexDirection: 'column',
    alignItems: 'center', justifyContent: 'flex-start', fontSize: '16px',
    fontWeight: 'bold', color: COLOR_CONSTANTS.TEXT,
  ...
  const mapsContentMinHeight = `${LAYOUT_CONSTANTS.MAPS_DETAIL_COL_MIN}px`;
  ...
  col2Content.dataset.cyclopediaRelaxedMinHeight = mapsContentMinHeight;
  col2Content.style.flex = '1 1 0';
  col2Content.style.minHeight = mapsContentMinHeight;
  col2Content.style.minWidth = '0';
  col2Content.style.width = '100%';
  col2Content.style.display = 'flex';
  col2Content.style.flexDirection = 'column';
  col2Content.style.overflowX = 'hidden';
  col2Content.style.overflowY = 'auto';
  col2Content.style.boxSizing = 'border-box';
  
  const col3Content = document.createElement('div');
  col3Content.className = 'maps-content-area';
  col3Content.dataset.cyclopediaRelaxedMinHeight = mapsContentMinHeight;
  col3Content.style.flex = '1 1 0';
  col3Content.style.minHeight = mapsContentMinHeight;
  col3Content.style.minWidth = '0';
  col3Content.style.width = '100%';
  col3Content.style.overflowX = 'auto';
  col3Content.style.overflowY = 'auto';
  col3Content.style.boxSizing = 'border-box';
  ...
  function updateRightCol() {
    try {
      ...
      const roomData = getCyclopediaRoomByMapId(selectedMap);
      ...
      if (selectedCategory && !selectedMap) {
        const region = findCyclopediaRegionById(selectedCategory);
        if (region?.rooms) {
          ...
          const mapsInRegion = getCyclopediaMapsInRegion(selectedCategory);

          const onRegionMapSelect = (mapId) => {
            selectedMap = mapId;
            cyclopediaState.mapsLastSelectedMapId = selectedMap;
            cyclopediaState.mapsLastSelectedRegionId = selectedCategory || null;
            updateBottomBox();
      ...
      if (selectedMap) {
        ...
        const statsContainer = createStatisticsSection(
          selectedMap,
          MapsTabDOMOptimizer.leaderboardBundle
        );
  ...
  rightCol.appendChild(col2);
  rightCol.appendChild(col3);

  rightCol._relayoutStackableColumns = setupCyclopediaStackableColumnsLayout({
    container: rightCol,
    stackThreshold: LAYOUT_CONSTANTS.MAPS_DETAIL_COL_MIN + LAYOUT_CONSTANTS.MAPS_STAT_COL_MIN,
    columns: [
      { el: col2 },
      { el: col3, noRightBorder: true }
    ],
    relaxScroll: () => [col2Content, col3Content]
  });
...
function createBestiaryTabPage(selectedCreature, selectedEquipment, selectedInventory, setSelectedCreature, setSelectedEquipment, setSelectedInventory, updateRightCol) {
  ...
  const { root: d, leftCol, rightCol } = createCyclopediaTabShell();
  ...
  const createCreatureBox = (title, items) => {
    ...
    Object.assign(box.style, CYCLOPEDIA_TAB_LAYOUT.box);
    ...
    if (title === 'Creatures') {
      ...
      if (titleEl) {
        ...
        buttonImg.src = CYCLOPEDIA_ASSETS.shinyStar;
        buttonImg.alt = 'shiny';
...
// --- 10d. Arsenal (Equipment) Tab ---

function createEquipmentTabPage(selectedCreature, selectedEquipment, selectedInventory, setSelectedCreature, setSelectedEquipment, setSelectedInventory, updateRightCol, arsenalTabNav = {}) {
  const { setActiveTab, tabPages } = arsenalTabNav;
  ...
  // Common styles for equipment tab detail columns (inside rightCol)
  ...
  rightCol.appendChild(equipDetailsCol);
  rightCol.appendChild(ownedEquipCol);
  ...
  function updateRightCol() {
    ...
    if (!currentSelectedEquipment) {
      ...
      let equipId = resolveCyclopediaEquipmentGameId(currentSelectedEquipment);
      ...
      if (equipId == null) {
        ...
        // Equipment details + usage stats (3-col grid: left | centered divider | right)
        const topRow = document.createElement('div');
        topRow.style.cssText =
          'display:grid; grid-template-columns:3fr 8px 2fr; grid-template-rows:auto auto minmax(0,1fr); gap:6px 0; width:100%; max-width:100%; align-items:stretch; min-height:0; flex:1 1 auto; box-sizing:border-box; padding:0 10px 8px;';

        const equipTopColLeft = 'grid-column:1; min-width:0;';
        const equipTopColRight = 'grid-column:3; min-width:0; padding-left:4px; box-sizing:border-box;';
        ...
        portrait.style.margin = '0';
        portrait.style.justifySelf = 'center';
        ...
        tooltipDiv.style.minWidth = '0';
        tooltipDiv.style.justifySelf = 'stretch';
        ...
        const tierSelector = createEquipmentTierSelector(updateRightCol);
        tierSelector.style.cssText += `${equipTopColLeft} margin:0; height:100%;`;
        topRow.appendChild(tierSelector);

        const usageTitle = document.createElement('div');
        usageTitle.className = FONT_CONSTANTS.SIZES.SMALL;
        usageTitle.style.cssText =
          `${equipTopColRight} font-weight:700; color:#ffe066; letter-spacing:0.02em; text-align:center; display:flex; align-items:center; justify-content:center; height:100%; width:100%;`;
        usageTitle.textContent = 'Player usage';
        topRow.appendChild(usageTitle);
        ...
        tierSeparator.style.cssText = `${equipTopColLeft} margin:0 0 4px 0;`;
        topRow.appendChild(tierSeparator);

        const usageSeparator = document.createElement('div');
        usageSeparator.className = 'separator my-2.5';
        usageSeparator.setAttribute('role', 'none');
        usageSeparator.style.cssText = `${equipTopColRight} margin:0 0 4px 0;`;
        topRow.appendChild(usageSeparator);

        const detailsBody = document.createElement('div');
        detailsBody.style.cssText =
          `${equipTopColLeft} display:grid; grid-template-columns:1fr; grid-template-rows:40px auto; gap:8px; min-height:0; width:100%;`;
        detailsBody.appendChild(portrait);
        detailsBody.appendChild(tooltipDiv);
        topRow.appendChild(detailsBody);

        const usageBody = document.createElement('div');
        usageBody.className = FONT_CONSTANTS.SIZES.SMALL;
        usageBody.style.cssText =
          `${equipTopColRight} text-align:left; color:#e6d7b0; display:flex; flex-direction:column; align-items:stretch; justify-content:center; overflow-y:auto; min-height:0; line-height:1.15; width:100%;`;
        ...
        if (usage.totalUsage <= 0) {
          ...
          usageBody.appendChild(usageCount);
          ...
          usageBody.appendChild(usagePct);
          ...
          usageBody.appendChild(usageDist);
          ...
          usageBody.appendChild(statRows);
        }

        topRow.appendChild(usageBody);

        const colDivider = document.createElement('div');
        colDivider.setAttribute('role', 'presentation');
        colDivider.style.cssText =
          'grid-column:2; grid-row:1/-1; width:1px; justify-self:center; align-self:stretch; background:#444; min-height:0;';
        topRow.appendChild(colDivider);
    ...
    if (!currentSelectedEquipment) {
      ...
      const equipId = resolveCyclopediaEquipmentGameId(currentSelectedEquipment);
  ...
  Object.assign(equipmentBox.style, { flex: '1 1 0', minHeight: '0' });
  ...
  rightCol._relayoutStackableColumns = setupCyclopediaStackableColumnsLayout({
    container: rightCol,
    stackThreshold: LAYOUT_CONSTANTS.EQUIPMENT_DETAIL_COL_MIN + LAYOUT_CONSTANTS.EQUIPMENT_OWNED_COL_MIN,
    columns: [
      { el: equipDetailsCol },
      { el: ownedEquipCol, noRightBorder: true }
    ],
    relaxScroll: () => [
      equipDetailsTop,
      equipDetailsBottom,
      ownedEquipCol.querySelector('div[style*="overflow-y"]')
    ]
  });
...
function createCharactersTabColumns(col1, rightCol) {
  Object.assign(col1.style, CYCLOPEDIA_TAB_LAYOUT.leftCol);
  Object.assign(rightCol.style, CYCLOPEDIA_TAB_LAYOUT.rightCol, CYCLOPEDIA_TAB_LAYOUT.rightColRow);

  const sharedScrollContainer = DOMUtils.createElement('div');
  Object.assign(sharedScrollContainer.style, {
    flex: '1 1 0',
    display: 'flex',
    flexDirection: 'row',
    height: '100%',
  ...
  rightCol.appendChild(sharedScrollContainer);

  return { col1, col2, col3, sharedScrollContainer, rightCol };
}
...
function createCharactersTabPage(selectedCreature, selectedEquipment, selectedInventory, setSelectedCreature, setSelectedEquipment, setSelectedInventory, updateRightCol) {
  const { root: d, leftCol: col1, rightCol } = createCyclopediaTabShell({ rightColRow: true });
  const { col2, col3, sharedScrollContainer } = createCharactersTabColumns(col1, rightCol);
  ...
  async function displayUserStats(selectedCategory) {
    ...
    try {
      ...
      if (selectedCategory !== 'Rankings') {
        ...
        CyclopediaApi.fetchRankings(rankingsPrefetchOptions).catch(() => {});
      }
      if (selectedCategory !== 'Rankings') {
      ...
      if (!profileData?.name) {
        profileData = await CyclopediaApi.fetchProfile(playerState.name, { priority: 1 });
  ...
  async function displayRankingsData(playerState) {
    ...
    try {
      ...
      let initialRankingsPromise = getCachedRankingsData(initialQueryOptions)
        ? null
        : CyclopediaApi.fetchRankings(initialQueryOptions);
      ...
      const rankingsPlayerCol = 'minmax(150px, 250px)';
      const rankingsGridColumns = `48px ${rankingsPlayerCol} 53px 53px 40px 48px 40px 40px 40px 40px 40px 40px 40px`;
      const rankingsTableMinWidth = '672px';

      const containerDiv = document.createElement('div');
      Object.assign(containerDiv.style, {
        display: 'flex', flexDirection: 'column', width: '100%', height: '100%',
        minHeight: '0', minWidth: '0', padding: '10px 0', boxSizing: 'border-box'
      });
      ...
      tableInner.style.cssText = `
        display: flex;
        flex-direction: column;
        width: 100%;
        min-width: ${rankingsTableMinWidth};
        max-width: 100%;
        height: 100%;
        min-height: 0;
        flex: 1 1 auto;
      ...
      const headerData = [
        ...
        { type: 'icon', src: CYCLOPEDIA_ASSETS.raid, alt: 'Raids', key: 'raids', sortable: true },
      ...
      const syncRankingsHeaderScrollbarPadding = () => {
        const gutter = Math.max(0, scrollableContainer.offsetWidth - scrollableContainer.clientWidth);
        headerRow.style.paddingRight = gutter > 0 ? `${gutter}px` : '0px';
      };
        ...
        const rowsHtml = currentRankings.map((ranking, index) => {
          ...
          return cellData.map((text, cellIndex) => {
            ...
            const playerAttrs = isPlayerCell
              ...
              : '';
            const layoutStyle = isPlayerCell
              ? 'display:block;'
            ...
            const playerStyle = isPlayerCell
              ...
              : '';
            return `<div class="${FONT_CONSTANTS.SIZES.SMALL}"${playerAttrs} style="${baseRankingsCellStyle}background:${rowBackground};font-weight:${isCurrentPlayer || isSearchedPlayer ? 'bold' : 'normal'};text-align:${textAlign};${layoutStyle}border:${rowBorder};${playerStyle}">${escapeRankingsText(text)}</div>`;
          }).join('');
        }).join('');
      ...
      async function loadRankings() {
        ...
        try {
          ...
          requestAnimationFrame(() => syncRankingsHeaderScrollbarPadding());
          updatePaginationControls();
      ...
      headerData.forEach((item) => {
        ...
        headerCell.style.cssText = `
          ...
          color: ${COLOR_CONSTANTS.PRIMARY};
          font-weight: bold;
          text-align: ${item.key === 'name' ? 'left' : 'center'};
          border: 4px solid transparent;
          border-image: ${rankingsCellFrame};
          background: rgba(0, 0, 0, 0.2);
          display: flex;
          align-items: center;
          justify-content: ${item.key === 'name' ? 'flex-start' : 'center'};
          font-family: 'Trebuchet MS', 'Arial Black', Arial, sans-serif;
          font-size: 10px;
          ${item.sortable ? 'cursor: pointer;' : 'cursor: default;'}
          transition: all 0.2s ease;
          ${item.key === 'name' ? 'min-width: 0; max-width: 100%; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; box-sizing: border-box;' : 'min-width: 0; box-sizing: border-box;'}
        `;
      ...
      tableInner.appendChild(scrollableContainer);
      horizontalScroll.appendChild(tableInner);
      tableContainer.appendChild(horizontalScroll);
      ...
      const rankingsDocumentClickHandler = (e) => {
      ...
      document.addEventListener('click', rankingsDocumentClickHandler);
      d.characterEventListeners.push({
        element: document,
        event: 'click',
        handler: rankingsDocumentClickHandler
      });
      ...
      const rankingsScrollbarObserver = new ResizeObserver(() => {
        syncRankingsHeaderScrollbarPadding();
      });
      rankingsScrollbarObserver.observe(scrollableContainer);
      d.characterEventListeners.push({ isObserver: true, observer: rankingsScrollbarObserver });
  ...
  function displayUserSpeedrunOrRankData(category, playerName, rooms, ROOM_NAMES, best, roomsHighscores, container, isTemplate = false) {
    const formatTime = FormatUtils.milliseconds;
    const containerDiv = document.createElement('div');
    Object.assign(containerDiv.style, {
      display: 'flex', flexDirection: 'column', width: '100%', height: '100%',
        ...
        getLocalRunsForMap(currentMapKey, category).then(localRuns => {
          if (localRuns && localRuns.length > 0) {
            ...
            localRuns.slice(0, 5).forEach((run, index) => {
              ...
              const value = category === 'speedrun' ? FormatUtils.ticks(run.time) : run.points;
              row.innerHTML = `
  ...
  function displayUserCombinedLeaderboardsData(playerName, rooms, ROOM_NAMES, best, roomsHighscores, container) {
    ...
    Object.entries(organizedRooms).forEach(([regionName, regionRooms]) => {
      ...
      yourRankData.innerHTML = `
        <span style="color: #8f8; font-weight: bold;">You:</span>
        <span style="white-space: nowrap;"><img src="${CYCLOPEDIA_ASSETS.starTier}" alt="Rank" style="width: 9px; height: 10px; vertical-align: middle; margin-right: 2px; display: inline-block;">${yourRank}${yourRankTicks !== undefined && yourRankTicks !== null ? ` <i style="color: #aaa;">(${yourRankTicks})</i>` : ' (null)'}</span>
      `;
      ...
      topRankData.innerHTML = `
        <span style="color: #ff8; font-weight: bold;">Top:</span>
        <span style="white-space: nowrap;"><img src="${CYCLOPEDIA_ASSETS.starTier}" alt="Rank" style="width: 9px; height: 10px; vertical-align: middle; margin-right: 2px; display: inline-block;">${topRank}${topRankTicks !== undefined && topRankTicks !== null ? ` <i style="color: #aaa;">(${topRankTicks})</i>` : ' (null)'}</span>
      `;
  ...
  function displayCombinedLeaderboardsSearchResults(playerName, searchedProfileData, yourRooms, ROOM_NAMES, best, roomsHighscores, container) {
    ...
    Object.entries(organizedRooms).forEach(([regionName, regionRooms]) => {
      ...
      searchedRankData.innerHTML = `
        <span style="color: ${COLOR_CONSTANTS.PRIMARY}; font-weight: bold;">${truncateListPlayerName(playerName)}:</span>
        <span style="white-space: nowrap;"><img src="${CYCLOPEDIA_ASSETS.starTier}" alt="Rank" style="width: 9px; height: 10px; vertical-align: middle; margin-right: 2px; display: inline-block;">${searchedRank !== null && searchedRank >= 0 ? searchedRank + (searchedRankTicks !== undefined && searchedRankTicks !== null ? ` <i style="color: #aaa;">(${searchedRankTicks})</i>` : ' (null)') : '-'}</span>
      `;
      ...
      topRankData.innerHTML = `
        <span style="color: #ff8; font-weight: bold;">Top:</span>
        <span style="white-space: nowrap;"><img src="${CYCLOPEDIA_ASSETS.starTier}" alt="Rank" style="width: 9px; height: 10px; vertical-align: middle; margin-right: 2px; display: inline-block;">${topRank !== null ? topRank + (topRankTicks !== undefined && topRankTicks !== null ? ` <i style="color: #aaa;">(${topRankTicks})</i>` : ' (null)') : '-'}</span>
      `;
        ...
        const performSearch = async () => {
          ...
          try {
            ...
            if (!searchedProfileData) {
              searchedProfileData = await CyclopediaApi.fetchProfile(searchTerm);

              if (searchedProfileData !== null && searchedProfileData !== undefined) {
            ...
            if (searchedProfileData) {
              ...
              if (playerState?.name) {
                ...
                if (!profileData) {
                  ...
                  // Fetch profile data
                  try {
                    profileData = await CyclopediaApi.fetchProfile(playerState.name);
                    if (profileData) setCachedProfileData(playerState.name, profileData);
                  } catch (error) {
            ...
            if (searchedProfileData) {
              ...
              if (playerState?.name) {
                ...
                displayCombinedLeaderboardsData(playerState).then(() => {
                  ...
                  CyclopediaApi.fetchProfile(cyclopediaState.searchedUsername).then(searchedProfileData => {
                    if (searchedProfileData !== null && searchedProfileData !== undefined) {
  ...
  // Defer profile fetch until Characters tab is first opened
  d._charactersTabActivated = false;
  d._activateCharactersTab = () => {
    if (d._charactersTabActivated) return;
    d._charactersTabActivated = true;
    displayUserStats('Player Information');
  };
  ...
  setupCyclopediaStackableColumnsLayout({
    container: sharedScrollContainer,
    stackThreshold: LAYOUT_CONSTANTS.CREATURE_DETAIL_FLEX_COL_MIN * 2,
    columns: [
      { el: col2 },
      { el: col3, noRightBorder: true }
    ],
  ...
  d.cleanupCharactersTab = () => {
    try {
      ...
      if (d.characterEventListeners) {
        d.characterEventListeners.forEach((entry) => {
          if (entry.isObserver && entry.observer) {
            try { entry.observer.disconnect(); } catch (error) { /* ignore */ }
          } else if (entry.element && entry.handler) {
...
function openCyclopediaModal(options) {
  ...
  (() => {
    try {
      ...
      activeModalLifecycle = ModalLifecycle.create();
    ...
    let setActiveTab;
    const tabPages = new Array(6).fill(null);

    function applyTabPageBaseStyles(page) {
    ...
    function ensureTabPage(index, mainContent) {
      if (tabPages[index]) return tabPages[index];
      const page = tabFactories[index]();
      tabPages[index] = page;
      if (activeModalLifecycle?.isActive) {
        activeModalLifecycle.registerTabPage(page);
      }
      applyTabPageBaseStyles(page);
      mainContent.appendChild(page);
      return page;
    }

    function defineSetActiveTab(tabButtons, mainContent) {
      ensureTabPage(activeTab, mainContent);
      ...
      setActiveTab = function(idx) {
        ...
        ensureTabPage(idx, mainContent);
        if (idx === 5) {
          tabPages[5]?._activateCharactersTab?.();
        }
        ...
        tabPages.forEach((page, i) => {
          if (page) {
            try {
              ...
              console.warn('[Cyclopedia] Error switching tab:', error);
            }
          }
        });
      };
    ...
    // Create equipment tab via factory (lazy with other tabs)
    const tabFactories = [
      () => createStartPage(),
      ...
      () => createMapsTabPage(selectedCreature, selectedEquipment, selectedInventory, v => { selectedCreature = v; }, v => { selectedEquipment = v; }, v => { selectedInventory = v; }, () => {}, { setActiveTab, tabPages }),
    ...
    try {
      ...
      const modalCleanupTimeout = setTimeout(() => {
        ...
        if (modalElement) {
          ...
          registerModalHandler(document, 'keydown', handleKeyDown);
          registerModalObserver(cleanupObserver, 'modalFallbackCleanup');
        }
      }, 100);
...
function closeCyclopediaModal() {
  try {
    if (activeCyclopediaModal?.close) {
      activeCyclopediaModal.close();
      return true;
    }
    cleanupCyclopediaModal();
    ...
    console.error('[Cyclopedia] Error closing modal:', error);
    try { cleanupCyclopediaModal(); } catch (_) { /* ignore */ }
    return false;
  }
}
...
function cleanupCyclopediaModal() {
  ...
  if (window.cyclopediaMenuObserver) {
    try {
      ...
      console.warn('[Cyclopedia] Error disconnecting menu observer:', error);
    }
  }
