...
function getDepotChestAssetUrl() {
  const assetPath = '/assets/ot/Depot_Chest.gif';
...
function isBattleHelperViewingForeignProfile() {
  try {
    return window.battleHelper?.isViewingForeignProfile?.() === true;
  } catch (_) {
    return false;
  }
}

// While Battle Helper has another player's arsenal loaded, all depot grid work is
// pointless (the grid shows a foreign roster) and just adds reflow churn on every
// tab switch. Suspend it until Battle Helper restores the original profile.
function isDepotWorkSuspended() {
  return isBattleHelperViewingForeignProfile();
}
...
function initDepotManager() {
  ...
  window.depotManager = {
    ...
    // Called by Battle Helper once it restores the original profile, so depot
    // layout re-syncs immediately instead of waiting for the next tab switch.
    resumeAfterForeignProfile: () => {
      if (!depotInitialized || isDepotWorkSuspended()) return;
      try {
        handleTabSwitchReapply();
      } catch (error) {
        console.error('[Depot Manager] resumeAfterForeignProfile error:', error);
      }
    },
