...
function createSettingsButton() {
  addOrShowHeaderNavItem({
    ...
    onReady: (li) => {
      ...
      placeSettingsBeforeCyclopedia(li);
    }
  });
}

// Keep the Settings nav <li> immediately before Cyclopedia's nav <li>.
// Both are mod-appended foreign nodes that sit after the game's own React
// children, so reordering them relative to each other stays on the React-safe
// side of the rule in .claude/CLAUDE.md (never move a *native* child). If
// Cyclopedia hasn't injected its button yet it appends after us later, which is
// already the desired order.
function placeSettingsBeforeCyclopedia(li) {
  try {
    const ul = li?.parentElement;
    if (!ul) return;
    const cyclopediaLi = ul.querySelector('.cyclopedia-header-btn')?.closest('li');
    if (!cyclopediaLi || cyclopediaLi === li) return;
    // Only act when we're currently positioned after Cyclopedia.
    if (li.compareDocumentPosition(cyclopediaLi) & Node.DOCUMENT_POSITION_PRECEDING) {
      ul.insertBefore(li, cyclopediaLi);
    }
  } catch (err) {
    console.error('[Mod Settings] placeSettingsBeforeCyclopedia failed:', err);
  }
}
