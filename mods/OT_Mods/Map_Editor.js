...
const CREATURE_COMBAT_FIELDS = [
  {
    ...
    labelKey: 'mods.mapEditor.creatureAbilityCd', labelFallback: 'Ability CD'
  },
  {
    formKey: 'attackCooldownTicks', datasetAttr: 'data-creature-attack-cd', unit: 'ticks',
    storeKey: 'attackCooldown', villainKey: 'attackCooldownTicks', liveKey: 'attack',
    labelKey: 'mods.mapEditor.creatureAttackCd', labelFallback: 'Attack CD'
  },
  {
...
function applyCooldownFieldToActorConfig(target, field, rawValue) {
  ...
  if (Number.isFinite(num) && num >= min) {
    ...
    delete target[field.storeKey];
  }
}

/**
 * The walk-cooldown ticks CustomBattles should enforce for a field's stored container.
 * `ticks` units pass through; `speed` units run the game's speed→step-duration formula.
 */
function cooldownFieldForwardTicks(container, field) {
  if (!container || typeof container !== 'object') return null;
  if (field.unit === 'speed') return nativeWalkCooldownTicksFromSpeed(container.speed);
  const v = Number(container.cooldownTicks);
...
/**
 * Outfit sprite field accepts a creature name/ID (resolved to that creature's own sprite),
 * or an explicit "outfit:<id>" to use a raw OUTFIT/<id>.png sprite that isn't necessarily
 * any creature's default look (e.g. "outfit:3" for https://bestiaryarena.com/assets/OUTFIT/3-moving.png).
 * The explicit form is required for that case — a bare number is resolved as a creature
 * gameId first (falling back to that same number only if no creature owns that gameId), so
 * "3" alone can silently resolve to a different creature's outfit instead of raw outfit 3.
 */
function resolveOutfitSpriteIdFromInput(input) {
  const trimmed = String(input ?? '').trim();
  const explicitMatch = /^outfit\s*:\s*(\d+)$/i.exec(trimmed);
  if (explicitMatch) {
...
/**
 * Display text for a resolved outfit sprite ID: the owning creature's name if one exists,
 * else the explicit "outfit:<id>" form (not a bare number — see resolveOutfitSpriteIdFromInput
 * for why a bare number would round-trip through creature-gameId resolution instead).
 */
function formatOutfitSpriteIdDisplay(spriteId) {
  return resolveCreatureNameFromOutfitSpriteId(spriteId) || `outfit:${spriteId}`;
}
...
function buildMapEditorVillainConfig(tileIndex, gameId, actorConfig = null) {
  ...
  CREATURE_COMBAT_FIELDS.forEach((field) => {
    ...
    if (ticks == null) {
      const flat = Number(actorConfig?.[field.villainKey]);
      if (Number.isFinite(flat) && flat >= 0) ticks = Math.floor(flat);
    }
    if (Number.isFinite(ticks) && ticks >= 0) config[field.villainKey] = ticks;
    // Also carry the raw SPD so the field round-trips through workshop/export JSON exactly
    // (ticks alone can't be inverted without loss).
    if (field.unit === 'speed') {
      const spd = Number(container?.speed ?? actorConfig?.moveSpeed);
      if (Number.isFinite(spd) && spd >= 1) config.moveSpeed = Math.floor(spd);
    }
  });
...
function normalizeActorConfig(raw) {
  ...
  CREATURE_COMBAT_FIELDS.forEach((field) => {
    ...
    const stored = container?.[cooldownFieldValueKey(field)];
    // Also accept a flat sibling key (villain-config round-trip): `<villainKey>` for tick
    // fields, `moveSpeed` for the speed field (its villainKey is ticks, wrong unit).
    const flat = field.unit === 'speed' ? normalized.moveSpeed : normalized[field.villainKey];
    applyCooldownFieldToActorConfig(normalized, field, stored ?? flat);
    delete normalized[field.villainKey];
  });
  if (normalized.abilityCooldown && normalized.ability?.cooldown) delete normalized.ability.cooldown;
  delete normalized.moveSpeed;
...
function computeCreatureEditorCombatStats(gameId, formState) {
  ...
  CREATURE_COMBAT_STAT_KEYS.forEach(({ key }) => {
    ...
    if (key === 'speed') {
      // Speed isn't gene/level-scaled; the "Move speed" Combat-timing field overrides it.
      const override = Number(formState.moveSpeed);
      stats[key] = Number.isFinite(override) && override >= 1 ? Math.floor(override) : baseValue;
      return;
    }
...
function buildCreatureEditorFormState(actor, tileIndex = null) {
  ...
  CREATURE_COMBAT_FIELDS.forEach((field) => {
    ...
    cooldownFormState[field.formKey] = readStoredCooldownFieldValue(container, field);
  });
  return {
    ...cooldownFormState,
...
function readLiveCreatureCooldowns(tileIndex) {
  ...
  const battle = mapEditorTestBattle;
  if (typeof battle?.readLiveActorBaseCooldownsForTile !== 'function') return null;
  try {
    return battle.readLiveActorBaseCooldownsForTile(
      tileIndex, !editorAlliedTiles.has(tileIndex)
    );
  } catch (_) {
    return null;
  }
...
/**
 * Native walk-cooldown ticks derived from a creature's base `speed`, replicating the game's
 * movement component: stepDurationMs = ⌊100000 / ⌊857.36·ln(speed/2 + 261.29) − 4794.51⌋⌋,
 * then ms→ticks at 62.5ms/tick. Verified against the live `_next` chunk.
 */
function nativeWalkCooldownTicksFromSpeed(speed) {
  const s = Number(speed);
  if (!Number.isFinite(s)) return null;
  let t = 1;
  if (s > -261.29) {
    t = Math.floor(857.36 * Math.log(s / 2 + 261.29) - 4794.51);
    if (t <= 0) t = 1;
  }
  const stepMs = Math.floor(100000 / t);
  const ticks = Math.round(stepMs / 62.5);
...
function nativeCreatureFieldValue(gameId, field) {
  ...
  if (!baseStats || typeof baseStats !== 'object') return null;
  if (field.unit === 'speed') {
    const v = Number(baseStats.speed);
    return Number.isFinite(v) && v >= 1 ? Math.floor(v) : null;
  }
  if (field.liveKey === 'attack') {
    const v = Number(baseStats.attackDelayTicks);
    return Number.isFinite(v) ? Math.max(3, Math.floor(v)) : null;
  }
  return null;
}
...
function appendCreatureCooldownSection(form, formState, tileIndex, actor) {
  ...
  CREATURE_COMBAT_FIELDS.forEach((field) => {
    ...
    // Ability with no cooldown clock (e.g. Cyclops' passive Groundshaker): a test battle
    // proved the live actor has no ability-cooldown component, so the field can't do
    // anything — disable it and say why.
    if (field.liveKey === 'ability' && live !== null && live.ability == null) {
      input.disabled = true;
      input.value = '';
      input.placeholder = '';
      const note = document.createElement('span');
      note.className = 'me-creature-form-hint';
      note.textContent = t('mods.mapEditor.creatureAbilityNoCooldownShort', 'no ability cooldown');
      row.appendChild(note);
      row.title = t(
        'mods.mapEditor.creatureAbilityNoCooldown',
        "This creature's ability has no cooldown to change."
      );
    }

    appendCreatureFormRow(form, t(field.labelKey, field.labelFallback), row);
  });
}
...
function createCreatureEditorPanel(tileIndex, actor) {
  ...
  appendCreatureCooldownSection(form, formState, tileIndex, actor);
  ...
  const outfitOwnIdBtn = createPanelButton(
    ...
    () => {
      ...
      outfitInput.value = formatOutfitSpriteIdDisplay(resolved);
      outfitInput.dispatchEvent(new Event('change', { bubbles: true }));
    },
