/**
 * Dashboard Script for Bestiary Arena Mod Loader
 * 
 * IMPORTANT: When adding a new mod, update the static mod lists in this file!
 * Dashboard cannot load ES6 modules, so we maintain static lists that must be kept in sync with:
 * - content/mod-registry.js (the source of truth)
 * - popup/popup.js (also uses static lists)
 * 
...
// Note: Dashboard cannot use ES6 imports, so we use direct message passing
...
async function scanAndRegisterLocalMods() {
  ...
  const modFiles = [
    ...
    // Super Mods - kept in sync with mod-registry.js
    // Note: Dashboard cannot load ES6 modules, so we maintain this list manually
    { name: 'Autoseller.js', enabled: true },
    { name: 'Autoscroller.js', enabled: true },
    { name: 'Better Analytics.js', enabled: true },
    { name: 'Better Boosted Maps.js', enabled: true },
    { name: 'Better Cauldron.js', enabled: true },
    { name: 'Better Exaltation Chest.js', enabled: true },
...
async function fetchAndRenderMods() {
  try {
    // Get mods directly from background script
...
if (resetAllBtn) resetAllBtn.addEventListener('click', debounce(async () => {
  ...
  try {
    // Clear extension storage - explicit keys first for Configurator compatibility
    if (window.browserAPI && window.browserAPI.storage && window.browserAPI.storage.local) {
      // Explicitly clear known Configurator storage keys
      const knownConfiguratorKeys = [
        'manualMods', 'localModsConfig', 'dashboard-theme', 'locale',
        'utility_script_cache', 'utility_script_timestamp', 'ba_local_runs'
      ];
      
      // Clear specific keys first
      await Promise.all(knownConfiguratorKeys.map(key => 
        new Promise(res => window.browserAPI.storage.local.remove([key], res))
      ));
      
      // Clear any script cache keys (script_*)
      const allLocalData = await new Promise(res => 
        window.browserAPI.storage.local.get(null, res)
      );
      const scriptCacheKeys = Object.keys(allLocalData).filter(key => key.startsWith('script_'));
      if (scriptCacheKeys.length > 0) {
        await new Promise(res => window.browserAPI.storage.local.remove(scriptCacheKeys, res));
      }
      
      // Clear any mod localStorage keys (mod_*, bestiary_*, ba_*)
      const modStorageKeys = Object.keys(allLocalData).filter(key => 
        key.startsWith('mod_') || key.startsWith('bestiary_') || key.startsWith('ba_')
      );
      if (modStorageKeys.length > 0) {
        await new Promise(res => window.browserAPI.storage.local.remove(modStorageKeys, res));
      }
      
      // Final clear of all remaining local storage
    ...
    // Clear extension's localStorage
    if (window.localStorage) {
      window.localStorage.clear();
    }
    
    // Clear game page's localStorage via content script
    try {
      ...
      if (tab && tab.url && tab.url.includes('bestiaryarena.com')) {
        await window.browserAPI.tabs.sendMessage(tab.id, {
          action: 'clearGameLocalStorage'
        });
      }
    } catch (e) {
      console.warn('Could not clear game localStorage:', e);
    }
    
    // Clear any in-memory caches from mods
    if (window.RunTrackerAPI && typeof window.RunTrackerAPI.clearCache === 'function') {
      window.RunTrackerAPI.clearCache();
    }
