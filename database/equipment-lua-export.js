...
function isRaidRoomId(roomId) {
  ...
  try {
    // Fallback only reached if mapsDatabase isn't loaded. ROOMS is a plain array, not
    // keyed by id — ROOMS?.[roomId] never resolves for a string id, so scan it properly.
