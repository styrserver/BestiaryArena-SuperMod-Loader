...
(() => {
	...
	try {
		...
		// Helper function to get current map and region information
		const getCurrentMapInfo = () => {
			try {
				...
				const selectedMap = boardSnapshot.context.selectedMap;
				...
				console.error('Error getting current map info:', error);
				return null;
			}
		};
		...
		const initializeMaps = () => {
			...
			try {
				if (safeAccess(globalThis, 'state.utils.ROOM_NAME')) {
					// Mutate in place (not `= new Map()`) — window.mapNamesToIds etc. below
					// hold a reference to these exact objects; reassigning would leave that
					// reference pointing at a stale, permanently-empty Map.
					mapNamesToIds.clear();
					...
					// Expected on the very first call of every page load, before game state
					// hydrates — setupMapInitialization() retries and recovers within ~1s.
					// console.log (not .warn): this isn't exceptional, it fires every time.
					console.log('state.utils.ROOM_NAME not available yet');
				}
			} catch (error) {
			...
			try {
				if (safeAccess(globalThis, 'state.utils.getMonster')) {
					// Mutate in place — see note above about window.monsterNamesToGameIds.
					monsterNamesToGameIds.clear();
					...
					console.log('state.utils.getMonster not available yet');
				}
			} catch (error) {
			...
			try {
				if (safeAccess(globalThis, 'state.utils.getEquipment')) {
					// Mutate in place — see note above about window.equipmentNamesToGameIds.
					equipmentNamesToGameIds.clear();
					...
					console.log('state.utils.getEquipment not available yet');
				}
			} catch (error) {
		...
		const configureBoard = (config) => {
			try {
				...
				// Handle map selection - make it more robust and future-proof
				let mapId = null;
				let selectedMap = null;
				
				// Method 1: Try to use the provided map name
				if (config.map && config.map !== 'undefined') {
					mapId = mapNamesToIds.get(config.map);
					if (mapId) {
						console.log(`Using provided map: ${config.map} (ID: ${mapId})`);
					} else {
						console.warn(`Unknown map name: ${config.map}, will try to use current map`);
					}
				}
				
				// Method 2: If no valid map provided, try to get current map from game state
				if (!mapId) {
					try {
						const boardSnapshot = globalThis.state.board.getSnapshot();
						if (boardSnapshot && boardSnapshot.context && boardSnapshot.context.selectedMap) {
							selectedMap = boardSnapshot.context.selectedMap;
							if (selectedMap.selectedRoom && selectedMap.selectedRoom.id) {
								mapId = selectedMap.selectedRoom.id;
								console.log(`Using current map from game state (ID: ${mapId})`);
							}
						}
					} catch (error) {
						console.warn('Could not get current map from game state:', error);
					}
				}
				
				// Method 3: If still no map, try to get the first available map
				if (!mapId && mapIdsToNames.size > 0) {
					// Get the first available map ID
					const firstMapId = mapIdsToNames.keys().next().value;
					if (firstMapId) {
						mapId = firstMapId;
						console.log(`Using first available map (ID: ${mapId})`);
					}
				}
				
				// If we still don't have a map, we can't proceed
				if (!mapId) {
					console.error('No valid map could be determined. Available maps:', Array.from(mapIdsToNames.keys()));
					throw new Error('No valid map could be determined. Please ensure you are in a game or provide a valid map name.');
				}
				...
				const observer = new MutationObserver((mutations) => {
					if (isStateReady()) {
						...
						if (result.mapsInitialized) {
							...
							observer.disconnect();
							// The setInterval fallback below is redundant once the observer
							// succeeds — without this it keeps ticking and re-runs the full
							// (expensive) initializeMaps() a second time on its next tick.
		...
		console.log('BA Sandbox Utils loaded successfully:', {
			...
			hasGetCurrentMapInfo: typeof window.$getCurrentMapInfo === 'function'
		});
	} catch (error) {
